...
VOID OutputDebugString( LPCSTR lpOutputString ) { LOGD( "%s", lpOutputString ); }
...
HANDLE CreateFile( LPCTSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPVOID lpSecurityAttributes, DWORD dwCreationDisposition,
                   DWORD dwFlagsAndAttributes, LPVOID hTemplateFile )
{
    ...
    if ( chooseCurrentKmlMode == ChooseKmlMode_FILE_OPEN || chooseCurrentKmlMode == ChooseKmlMode_FILE_OPEN_WITH_FOLDER ) {
        ...
        if ( foundDocumentScheme ) {
            ...
            // and we extract the folder URL with "content://" scheme (in the variable "szEmuDirectory"/"szRomDirectory" and
            // "szCurrentContentDirectory") which should contain the script and its dependencies like the includes, the images and the ROMs.
    ...
    if ( !forceNormalFile && ( szCurrentAssetDirectory || _tcsncmp( lpFileName, assetsPrefix, assetsPrefixLength ) == 0 ) &&
         !foundDocumentScheme && !urlContentSchemeFound ) {
...
BOOL ReadFile( HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped )
{
    ...
    if ( hFile->handleType == HANDLE_TYPE_FILE ) {
        ...
        SERIAL_LOGD( "ReadFile(hFile: %p, lpBuffer: 0x%08x, nNumberOfBytesToRead: %d) -> %d bytes\n%s", hFile, lpBuffer,
                     nNumberOfBytesToRead, readByteCount, hexAsciiDump );
        free( hexAsciiDump );
#endif
    }
...
BOOL WriteFile( HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten, LPOVERLAPPED lpOverlapped )
{
    ...
    if ( hFile->handleType == HANDLE_TYPE_FILE ) {
        ...
        SERIAL_LOGD( "WriteFile(hFile: %p, lpBuffer: 0x%08x, nNumberOfBytesToWrite: %d) -> %d bytes\n%s", hFile, lpBuffer,
                     nNumberOfBytesToWrite, writenByteCount, hexAsciiDump );
        free( hexAsciiDump );
#endif
...
HANDLE CreateFileMapping( HANDLE hFile, LPSECURITY_ATTRIBUTES lpFileMappingAttributes, DWORD flProtect, DWORD dwMaximumSizeHigh,
...
{
    FILE_LOGD( "MapViewOfFile(hFileMappingObject: %p, dwDesiredAccess: 0x%08x, dwFileOffsetLow: %d)", hFileMappingObject, dwDesiredAccess,
               dwFileOffsetLow );
    ...
    if ( hFileMappingObject->handleType == HANDLE_TYPE_FILE_MAPPING ) {
        ...
        if ( dwDesiredAccess & FILE_MAP_WRITE )
            prot |= PROT_WRITE;
        hFileMappingObject->fileMappingAddress = mmap( NULL, hFileMappingObject->fileMappingSize, prot, MAP_PRIVATE,
        ...
        DWORD readByteCount =
            ( DWORD )read( hFileMappingObject->fileDescriptor, hFileMappingObject->fileMappingAddress, numberOfBytesToRead );
        ...
        hFileMappingObject->fileMappingAddress =
            ( LPVOID )( AAsset_getBuffer( hFileMappingObject->fileAsset ) + hFileMappingObject->fileMappingOffset );
    }
...
void DeleteCriticalSection( CRITICAL_SECTION* lpCriticalSection ) { pthread_mutex_destroy( lpCriticalSection ); }

void EnterCriticalSection( CRITICAL_SECTION* lock ) { pthread_mutex_lock( lock ); }

void LeaveCriticalSection( CRITICAL_SECTION* lock ) { pthread_mutex_unlock( lock ); }

DWORD GetPrivateProfileString( LPCTSTR lpAppName, LPCTSTR lpKeyName, LPCTSTR lpDefault, LPTSTR lpReturnedString, DWORD nSize,
                               LPCTSTR lpFileName )
{
    // TODO
#ifdef UNICODE
...
HGLOBAL WINAPI GlobalAlloc( UINT uFlags, SIZE_T dwBytes ) { return malloc( dwBytes ); }
LPVOID WINAPI GlobalLock( HGLOBAL hMem ) { return hMem; }

BOOL WINAPI GlobalUnlock( HGLOBAL hMem ) { return TRUE; }
...
{
    if ( openFilename ) {
        if ( currentDialogBoxMode == DialogBoxMode_SET_USRPRG32 || currentDialogBoxMode == DialogBoxMode_SET_USRPRG42 ||
...
HANDLE LoadImage( HINSTANCE hInst, LPCSTR name, UINT type, int cx, int cy, UINT fuLoad )
{
    ...
    if ( ReadFile( hIconFile, iconHeaderArray, iconsBufferSize, &nNumberOfBytesToRead, NULL ) == FALSE ||
         iconsBufferSize != nNumberOfBytesToRead ) {
        CloseHandle( hIconFile );
        return NULL;
    }
...
{
    // TODO
    if ( currentDialogBoxMode == DialogBoxMode_GET_USRPRG32 || currentDialogBoxMode == DialogBoxMode_GET_USRPRG42 ||
         currentDialogBoxMode == DialogBoxMode_SET_USRPRG32 || currentDialogBoxMode == DialogBoxMode_SET_USRPRG42 ) {
        ...
        if ( lParam != NULL ) {
            ...
            if ( hIcon->handleType == HANDLE_TYPE_ICON && hIcon->icon != NULL ) {
                ...
                if ( icon && icon->bitmapInfoHeader && icon->bitmapBits )
                    setKMLIcon( icon->bitmapInfoHeader->biWidth, icon->bitmapInfoHeader->biHeight, icon->bitmapBits,
                                icon->bitmapInfoHeader->biSizeImage );
            }
        } else {
...
void bqPlayerCallback( SLAndroidSimpleBufferQueueItf bq, void* context )
{
    ...
    if ( hwo->pWaveHeaderNext != NULL ) {
        ...
        if ( pWaveHeaderNext != NULL ) {
            ...
            SLresult result = ( *hwo->bqPlayerBufferQueue )
                                  ->Enqueue( hwo->bqPlayerBufferQueue, pWaveHeaderNext->lpData, pWaveHeaderNext->dwBufferLength );
...
MMRESULT waveOutOpen( LPHWAVEOUT phwo, UINT uDeviceID, LPCWAVEFORMATEX pwfx, DWORD_PTR dwCallback, DWORD_PTR dwInstance, DWORD fdwOpen )
{
    ...
    result =
        ( *handle->engineEngine )->CreateAudioPlayer( handle->engineEngine, &handle->bqPlayerObject, &audioSrc, &audioSnk, 2, ids2, req2 );
    if ( result != SL_RESULT_SUCCESS ) {
            ...
            case HGDIOBJ_TYPE_BITMAP:
                if ( h && c == sizeof( BITMAP ) && pv ) {
                    ...
                    pBITMAP->bmWidthBytes =
                        ( 4 * ( ( hBITMAP->bitmapInfoHeader->biWidth * hBITMAP->bitmapInfoHeader->biBitCount + 31 ) / 32 ) );
...
BOOL PatBlt( HDC hdcDest, int x, int y, int w, int h, DWORD rop )
{
    ...
    if ( ( hdcDest->selectedBitmap || hdcDest->hdcCompatible == NULL ) && w && h ) {
        ...
        PALETTEENTRY* palPalEntry =
            palette && palette->paletteLog && palette->paletteLog->palPalEntry ? palette->paletteLog->palPalEntry : NULL;
...
{
    PAINT_LOGD( "PAINT StretchBlt(hdcDest: %p, xDest: %d, yDest: %d, wDest: %d, hDest: %d, hdcSrc: %p, xSrc: %d, ySrc: %d, wSrc: %d, hSrc: "
                "%d, rop: 0x%08x)",
                hdcDest, xDest, yDest, wDest, hDest, hdcSrc, xSrc, ySrc, wSrc, hSrc, rop );

    if ( hdcDest && hdcSrc && ( hdcDest->selectedBitmap || hdcDest->hdcCompatible == NULL ) && hdcSrc->selectedBitmap && hDest && hSrc ) {
        ...
        // LOGD("StretchBlt(%p, x:%d, y:%d, w:%d, h:%d, %08x, x:%d, y:%d, w:%d, h:%d) -> sourceBitCount: %d", hdcDest->hdcCompatible, xDest,
        // yDest, wDest, hDest, hdcSrc, xSrc, ySrc, wSrc, hSrc, sourceBitCount);
        ...
        StretchBltInternal( xDest, yDest, wDest, hDest, pixelsDestination, destinationBitCount, destinationStride, destinationWidth,
                            destinationHeight, xSrc, ySrc, wSrc, hSrc, pixelsSource, sourceBitCount, sourceStride, sourceWidth,
                            sourceHeight, rop, sourceTopDown, destinationTopDown, palPalEntry, brushColor, backgroundColor );
...
void StretchBltInternal( int xDest, int yDest, int wDest, int hDest, const void* pixelsDestination, int destinationBitCount,
                         int destinationStride, int destinationWidth, int destinationHeight, int xSrc, int ySrc, int wSrc, int hSrc,
                         const void* pixelsSource, UINT sourceBitCount, int sourceStride, int sourceWidth, int sourceHeight, DWORD rop,
                         BOOL sourceTopDown, BOOL destinationTopDown, const PALETTEENTRY* palPalEntry, COLORREF brushColor,
...
||||||| parent of fe0c675 (dos2unix and clang-format emu48plus sources when pulling them)
            switch (destinationBitCount) {
                case 1: {
                    //TODO https://devblogs.microsoft.com/oldnewthing/?p=29013
                    // If you blit from a color DC to a monochrome DC,
                ...
                case 32:
                    {
                        ...
                        break;
>>>>>>> fe0c675 (dos2unix and clang-format emu48plus sources when pulling them)
                    }
                default:
                    break;
            }
...
{
    if ( prgbq && hdc && hdc->realizedPalette && hdc->realizedPalette->paletteLog && hdc->realizedPalette->paletteLog->palPalEntry &&
...
static BOOL DecodeRleBmp( LPBYTE ppvBits, BITMAPINFOHEADER CONST* lpbi, LPBMPFILE pBmp )
{
    ...
    do {
        ...
        if ( ReadRleBmpByte( pBmp, &byLength ) )
            return TRUE;
        if ( ReadRleBmpByte( pBmp, &byColorIndex ) )
            return TRUE;
                ...
                default: // absolute mode
                    ...
                    if ( lpbi->biBitCount == 4 ) // RLE4
                    {
                        ...
                        for ( s = 0; s < byColorIndex; ++s ) {
                            ...
                            {
                                // fetch color pair
                                if ( ReadRleBmpByte( pBmp, &byColorPair ) )
                                    return TRUE;
                    ...
                    {
                        if ( pBmp->dwPos + byColorIndex > pBmp->dwFileSize )
                            return TRUE;
                ...
                case BI_RLE8:
                    {
                        ...
                        size_t stride =
                            ( size_t )( 4 * ( ( newBitmapInfo->bmiHeader.biWidth * newBitmapInfo->bmiHeader.biBitCount + 31 ) / 32 ) );
                        size_t size = abs( newBitmapInfo->bmiHeader.biHeight ) * stride;
                        bitmapBits = malloc( size );
                        BMPFILE Bmp;
                        int bitOffset = sizeof( BITMAPINFOHEADER ) + ( pbmi->bmiHeader.biCompression == BI_BITFIELDS
                                                                           ? 3 * sizeof( DWORD )
                ...
                case BI_BITFIELDS:
                    {
                        ...
                        size_t stride =
                            ( size_t )( 4 * ( ( newBitmapInfo->bmiHeader.biWidth * newBitmapInfo->bmiHeader.biBitCount + 31 ) / 32 ) );
                        size_t size = newBitmapInfo->bmiHeader.biSizeImage ? newBitmapInfo->bmiHeader.biSizeImage
...
HBITMAP CreateDIBSection( HDC hdc, CONST BITMAPINFO* pbmi, UINT usage, VOID** ppvBits, HANDLE hSection, DWORD offset )
{
    ...
    size_t stride = ( size_t )( 4 * ( ( newBitmapInfo->bmiHeader.biWidth * newBitmapInfo->bmiHeader.biBitCount + 31 ) / 32 ) );
    size_t size =
        newBitmapInfo->bmiHeader.biSizeImage ? newBitmapInfo->bmiHeader.biSizeImage : abs( newBitmapInfo->bmiHeader.biHeight ) * stride;
            ...
            case 4:
                {
                    ...
                    if ( palPalEntry ) {
                        resultColor = 0xFF000000 | RGB( palPalEntry[ colorIndex ].peRed, palPalEntry[ colorIndex ].peGreen,
...
BOOL WINAPI OpenClipboard( HWND hWndNewOwner ) { return TRUE; }
...
BOOL WINAPI EmptyClipboard( VOID ) { return TRUE; }
...
static void dumpTimers()
{
    ...
    for ( struct timerEvent* nextTimer = timersList.next; nextTimer != &timersList; nextTimer = nextTimer->next ) {
        TIMER_LOGD( "\ttimerId: %d (%x), uDelay: %d, dwUser: %d, fuEvent: %d, triggerTime: %ld)", nextTimer->timerId, nextTimer,
                    nextTimer->uDelay, nextTimer->dwUser, nextTimer->fuEvent, nextTimer->triggerTime );
    }
}
...
INT_PTR DialogBoxParam( HINSTANCE hInstance, LPCTSTR lpTemplateName, HWND hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam )
{
    if ( lpTemplateName == MAKEINTRESOURCE( IDD_CHOOSEKML ) ) {
        ...
        } else if ( currentDialogBoxMode == DialogBoxMode_SET_USRPRG32 || currentDialogBoxMode == DialogBoxMode_SET_USRPRG42 ) {
            itemDataCount = 0;
            lpDialogFunc( NULL, WM_INITDIALOG, 0, 0 );
...
int WINAPI wvsprintf( LPWSTR, LPCWSTR, va_list arglist ) { return vswprintf( arg1, MAX_PATH, arg2, arglist ); }
DWORD GetFullPathName( LPCWSTR lpFileName, DWORD nBufferLength, LPWSTR lpBuffer, LPWSTR* lpFilePart ) { return 0; }
LPWSTR lstrcpyn( LPWSTR lpString1, LPCWSTR lpString2, int iMaxLength ) { return strcpy( lpString1, lpString2 ); }
LPWSTR lstrcat( LPWSTR lpString1, LPCWSTR lpString2 ) { return NULL; }
void __cdecl _wsplitpath( const wchar_t* _FullPath, wchar_t* _Drive, wchar_t* _Dir, wchar_t* _Filename, wchar_t* _Ext ) {}
int WINAPI lstrcmp( LPCWSTR lpString1, LPCWSTR lpString2 ) { return wcscmp( lpString1, lpString2 ); }
int lstrcmpi( LPCWSTR lpString1, LPCWSTR lpString2 ) { return wcscasecmp( lpString1, lpString2 ); }
void _wmakepath( wchar_t _Buffer, const wchar_t* _Drive, const wchar_t* _Dir, const wchar_t* _Filename, const wchar_t* _Ext ) {}

#else

int WINAPI wvsprintf( LPSTR arg1, LPCSTR arg2, va_list arglist ) { return vsprintf( arg1, arg2, arglist ); }
...
LPSTR lstrcpyn( LPSTR lpString1, LPCSTR lpString2, int iMaxLength ) { return strcpy( lpString1, lpString2 ); }
LPSTR lstrcat( LPSTR lpString1, LPCSTR lpString2 ) { return strcat( lpString1, lpString2 ); }
...
int WINAPI lstrcmp( LPCSTR lpString1, LPCSTR lpString2 ) { return strcmp( lpString1, lpString2 ); }
int lstrcmpi( LPCSTR lpString1, LPCSTR lpString2 ) { return strcasecmp( lpString1, lpString2 ); }
