 ...
 *
 *   Copyright (C) 2004 Christoph Giesselink
 *
 */
#include "pch.h"
#include "resource.h"
...
static DWORD WINAPI EventThread( LPVOID pParam )
{
    ...
    while ( WaitForSingleObject( hEventPlay, dwTime ) == WAIT_TIMEOUT ) {
        ...
        while ( TRUE ) {
            ...
            if ( !ReadFile( hMacroFile, &dwData, sizeof( dwData ), &dwRead, NULL ) || dwRead != sizeof( dwData ) ) {
...
LRESULT OnToolMacroNew( VOID )
{
    ...
    InitializeOFN( &ofn );
    ofn.lpstrFilter = _T("Keyboard Macro Files (*.MAC)\0*.MAC\0")
                      _T("All Files (*.*)\0*.*\0");
    ...
    hMacroFile = CreateFile( szMacroFile, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
    if ( hMacroFile == INVALID_HANDLE_VALUE )
        return 0;
    ...
    MessageBox( hWnd, _T("Press OK to begin to record the Macro."), _T("Macro Recorder"), MB_OK | MB_ICONINFORMATION );
...
LRESULT OnToolMacroPlay( VOID )
{
    ...
    hMacroFile = CreateFile( szMacroFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
    if ( hMacroFile == INVALID_HANDLE_VALUE )
        return 0;

    // read header
    ReadFile( hMacroFile, byHeader, sizeof( byHeader ), &dwRead, NULL );
    if ( dwRead != sizeof( byHeader ) || memcmp( byHeader, KEYMACROHEAD, dwRead ) != 0 ) {
        MessageBox( hWnd, _T("Wrong keyboard macro file format."), _T("Macro Recorder"), MB_OK | MB_ICONSTOP );
