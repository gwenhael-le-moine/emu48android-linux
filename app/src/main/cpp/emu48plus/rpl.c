...
#define AVMEM                                                                                                                              \
    ( ( cCurrentRomType != 'X' && cCurrentRomType != '2' && cCurrentRomType != 'Q' ) ? ( ( cCurrentRomType == 'S' ) ? 0x7066E : 0x807ED )  \
                                                                                     : 0x80E9B )
#define INTRPPTR                                                                                                                           \
    ( ( cCurrentRomType != 'X' && cCurrentRomType != '2' && cCurrentRomType != 'Q' ) ? ( ( cCurrentRomType == 'S' ) ? 0x705B0 : 0x8072F )  \
                                                                                     : 0x8076B )
#define SYSTEMFLAGS                                                                                                                        \
    ( ( cCurrentRomType != 'X' && cCurrentRomType != '2' && cCurrentRomType != 'Q' ) ? ( ( cCurrentRomType == 'S' ) ? 0x706C5 : 0x80843 )  \
                                                                                     : 0x80F02 )
    ...
    if ( cCurrentRomType == 'G' && Port1 && Chipset.cards_status & PORT1_PRESENT ) {
        ...
        if ( !strncmp( ( LPCSTR )&Port1[ 12 ], "\xD\x4\x4\x4\x7\x4\xB\x4\x5\x4\x2\x5\xA\x3", 14 ) ) {
            ...
            if ( !strncmp( ( LPCSTR )&Port1[ 26 ], "\xD\x4\xB\x4", 4 ) ) {
                // get version number
                WORD wVersion = ( ( Port1[ 30 ] * 10 ) + Port1[ 34 ] ) * 10 + Port1[ 36 ];
...
static DWORD RPL_GarbageCol( VOID ) // RPL variables must be in system RAM
{
    ...
    _ASSERT( cCurrentRomType == 'S' || cCurrentRomType == 'G' || cCurrentRomType == 'X' || cCurrentRomType == '2' ||
             cCurrentRomType == 'Q' );
...
DWORD RPL_SkipOb( DWORD d )
{
    ...
    switch ( n ) {
        case DOFLASHP:
            l = ( cCurrentRomType != 'X' && cCurrentRomType != '2' && cCurrentRomType != 'Q' ) ? 5 : 12;
            break; // Flash PTR (HP49G)   // CdB for HP: add apples
        case DOBINT:
            l = 10;
            break; // System Binary
        case DOREAL:
...
#define SERIES49 ( cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'Q' )
    ...
    if ( s < 5 )
        return BAD_OB; // size too small for prolog
    n = Npack( o, 5 ); // read prolog
    switch ( n ) {
        case DOFLASHP:
            l = ( SERIES49 ) ? 12 : 5;
            break; // Flash PTR (HP49G)
        case DOBINT:
            l = 10;
            break; // System Binary
        case DOREAL:
        ...
        case DOCOL:  // Program
            ...
            do {
                l += n;
                if ( l > s )
                    return BAD_OB;                  // prevent negative size argument
                n = RPL_ObjectSize( o + l, s - l ); // get new object
                if ( n == BAD_OB )
                    return BAD_OB; // buffer overflow
            } while ( n );
            l += 5;
            break;
        case SEMI:
...
DWORD RPL_CreateTemp( DWORD l, BOOL bGarbageCol )
{
    ...
    if ( ( b + l ) > c )
        return 0;                       // check if now there's enough memory to move DSKTOP
...
UINT RPL_Depth( VOID ) { return ( Read5( EDITLINE ) - Read5( DSKTOP ) ) / 5 - 1; }
...
DWORD RPL_Pick( UINT l )
{
    ...
    if ( l == 0 )
        return 0;
    if ( METAKERNEL )
        ++l; // Metakernel support
    if ( RPL_Depth() < l )
        return 0; // not enough elements on stack
...
VOID RPL_Push( UINT l, DWORD n )
{
    ...
    if ( l > RPL_Depth() + 1 )
        return; // invalid stack level

    avmem = Read5( AVMEM ); // amount of free memory
    if ( avmem == 0 )
        return;             // no memory free
    avmem--;                // fetch memory
    Write5( AVMEM, avmem ); // save new amount of free memory

    if ( METAKERNEL )
        ++l; // Metakernel, save MK object on stack level 1
