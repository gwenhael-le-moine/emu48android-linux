...
static __inline UINT MIN( UINT a, UINT b ) { return ( a < b ) ? a : b; }

static __inline UINT MAX( UINT a, UINT b ) { return ( a > b ) ? a : b; }
...
static __inline BYTE UckBit( BYTE byBaudIndex )
{
    ...
    return ( ( ( BYTE )( ( ( lLC.QuadPart - lAppStart.QuadPart ) * dwBaudrates[ byBaudIndex ] ) / lFreq.QuadPart ) & 0x1 ) << 3 );
}
...
VOID Map( BYTE a, BYTE b ) // maps 2KB pages with priority
{
    ...
    _ASSERT( ( cCurrentRomType != 'E' && cCurrentRomType != 'X' && cCurrentRomType != 'P' && cCurrentRomType != '2' &&
               cCurrentRomType != 'Q' ) ||
             !Chipset.P1Cfig || Chipset.cards_status == 0xF ); // CdB for HP: add apples
    ...
    {
        if ( Chipset.BSCfig )
            MapBS( a, b ); // NCE3, not used in S(X)
        if ( Chipset.P1Cfig )
            MapP1( a, b ); // CE1, port1 (lower priority than CE2)
        if ( Chipset.P2Cfig )
            MapP2( a, b ); // CE2, port2 (higher priority than CE1)
    } else                 // HP48GX / HP49G
    {
        ...
        if ( Chipset.BSCfig )
            MapBS( a, b ); // CE1, bank select (lower priority than CE2)
        if ( Chipset.P1Cfig )
            MapP1( a, b ); // CE2, port1 (higher priority than CE1)
    }
    if ( Chipset.P0Cfig )
        MapP0( a, b ); // RAM, highest priority (execpt HDW)
    // CdB for HP: add apples header
    // @todo cg, bug if display header area is mapped to addr 0
    if ( Chipset.d0address != 0 ) {
        RMap[ Chipset.d0address ] = &( Chipset.d0memory[ 0 ] );
        RMap[ Chipset.d0address + 1 ] = &( Chipset.d0memory[ 2048 * 2 ] );
        WMap[ Chipset.d0address ] = &( Chipset.d0memory[ 0 ] );
        WMap[ Chipset.d0address + 1 ] = &( Chipset.d0memory[ 2048 * 2 ] );
    }
    return;
}
...
{
    // only HP39/40G, HP49G
    if ( cCurrentRomType == 'E' || cCurrentRomType == 'X' || cCurrentRomType == 'P' || cCurrentRomType == '2' ||
         cCurrentRomType == 'Q' ) // CdB for HP: add apples
    {
...
VOID Uncnfg()
{
    ...
    if ( ( Chipset.IOCfig ) && ( ( d & 0xFFFC0 ) == Chipset.IOBase ) ) {
        Chipset.IOCfig = FALSE;
        Map( b, b );
        return;
    }
    if ( ( Chipset.P0Cfig ) && ( ( b & ~Chipset.P0Size ) == P0MAPBASE ) ) {
        Chipset.P0Cfig = FALSE;
        Chipset.P0Cfg2 = FALSE;
        Map( P0MAPBASE, Chipset.P0End );
        return;
    }
    if ( cCurrentRomType == 'S' ) // HP48SX
    {
        if ( ( Chipset.P2Cfig ) && ( ( b & ~Chipset.P2Size ) == P2MAPBASE ) ) {
            Chipset.P2Cfig = FALSE;
            Chipset.P2Cfg2 = FALSE;
            Map( P2MAPBASE, Chipset.P2End );
            return;
        }
        if ( ( Chipset.P1Cfig ) && ( ( b & ~Chipset.P1Size ) == P1MAPBASE ) ) {
            Chipset.P1Cfig = FALSE;
            Chipset.P1Cfg2 = FALSE;
            Map( P1MAPBASE, Chipset.P1End );
            return;
        }
        if ( ( Chipset.BSCfig ) && ( ( b & ~Chipset.BSSize ) == BSMAPBASE ) ) {
            Chipset.BSCfig = FALSE;
            Chipset.BSCfg2 = FALSE;
            Map( BSMAPBASE, Chipset.BSEnd );
            return;
        }
    } else // HP48GX / HP49G
    {
...
VOID C_Eq_Id()
{
    // config order is HDW, RAM, CE1, CE2, NCE3
    if ( !Chipset.IOCfig ) {
        Nunpack( Chipset.C, ( Chipset.IOBase ) ^ 0x00019, 5 );
        return;
    }
    if ( !Chipset.P0Cfg2 ) {
        Nunpack( Chipset.C, ( Chipset.P0Size * 0x1000 ) ^ 0xFF003, 5 );
        return;
    }
    if ( !Chipset.P0Cfig ) {
        Nunpack( Chipset.C, ( Chipset.P0Base * 0x1000 ) ^ 0x000F4, 5 );
        return;
    }
    if ( cCurrentRomType == 'S' ) // HP48SX
    {
        if ( !Chipset.P1Cfg2 ) {
            Nunpack( Chipset.C, ( Chipset.P1Size * 0x1000 ) ^ 0xFF005, 5 );
            return;
        }
        if ( !Chipset.P1Cfig ) {
            Nunpack( Chipset.C, ( Chipset.P1Base * 0x1000 ) ^ 0x000F6, 5 );
            return;
        }
        if ( !Chipset.P2Cfg2 ) {
            Nunpack( Chipset.C, ( Chipset.P2Size * 0x1000 ) ^ 0xFF007, 5 );
            return;
        }
        if ( !Chipset.P2Cfig ) {
            Nunpack( Chipset.C, ( Chipset.P2Base * 0x1000 ) ^ 0x000F8, 5 );
            return;
        }
        if ( !Chipset.BSCfg2 ) {
            Nunpack( Chipset.C, ( Chipset.BSSize * 0x1000 ) ^ 0xFF001, 5 );
            return;
        }
        if ( !Chipset.BSCfig ) {
            Nunpack( Chipset.C, ( Chipset.BSBase * 0x1000 ) ^ 0x000F2, 5 );
            return;
        }
    } else // HP48GX / HP49G
    {
        if ( !Chipset.BSCfg2 ) {
            Nunpack( Chipset.C, ( Chipset.BSSize * 0x1000 ) ^ 0xFF005, 5 );
            return;
        }
        if ( !Chipset.BSCfig ) {
            Nunpack( Chipset.C, ( Chipset.BSBase * 0x1000 ) ^ 0x000F6, 5 );
            return;
        }
        if ( !Chipset.P1Cfg2 ) {
            Nunpack( Chipset.C, ( Chipset.P1Size * 0x1000 ) ^ 0xFF007, 5 );
            return;
        }
        if ( !Chipset.P1Cfig ) {
            Nunpack( Chipset.C, ( Chipset.P1Base * 0x1000 ) ^ 0x000F8, 5 );
            return;
        }
        if ( !Chipset.P2Cfg2 ) {
            Nunpack( Chipset.C, ( Chipset.P2Size * 0x1000 ) ^ 0xFF001, 5 );
            return;
        }
        if ( !Chipset.P2Cfig ) {
            Nunpack( Chipset.C, ( Chipset.P2Base * 0x1000 ) ^ 0x000F2, 5 );
            return;
        }
    }
...
enum MMUMAP MapData( DWORD d ) // check MMU area
{
    ...
    if ( Chipset.IOCfig && ( ( d & 0xFFFC0 ) == Chipset.IOBase ) )
        return M_IO;
    if ( Chipset.P0Cfig && ( ( ( u ^ Chipset.P0Base ) & ~Chipset.P0Size ) == 0 ) )
        return M_RAM;
    if ( cCurrentRomType == 'S' ) {
        if ( Chipset.P2Cfig && ( ( ( u ^ Chipset.P2Base ) & ~Chipset.P2Size ) == 0 ) )
            return M_P2;
        if ( Chipset.P1Cfig && ( ( ( u ^ Chipset.P1Base ) & ~Chipset.P1Size ) == 0 ) )
            return M_P1;
        if ( Chipset.BSCfig && ( ( ( u ^ Chipset.BSBase ) & ~Chipset.BSSize ) == 0 ) )
            return M_BS;
    } else {
        if ( Chipset.P1Cfig && ( ( ( u ^ Chipset.P1Base ) & ~Chipset.P1Size ) == 0 ) )
            return M_P1;
...
VOID Npeek( BYTE* a, DWORD d, UINT s )
{
    ...
    do {
        ...
        if ( M_IO == eMap )  // I/O access
        {
            ...
            if ( ( cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'P' || cCurrentRomType == 'Q' ) &&
                 ( Chipset.IORam[ LCR ] & LED ) && M_P2 == eMap ) // CdB for HP: add apples
            {
    ...
    do {
        ...
        if ( M_IO == eMap )  // I/O access
        {
            ...
            if ( cCurrentRomType != 'S' && M_BS == eMap ) {
                ...
                if ( cCurrentRomType == 'E' || cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'P' ||
                     cCurrentRomType == 'Q' ) // HP39/40G, HP49G  // CdB for HP: add apples
                {
                    RomSwitch( v + c );
                }
            }
        ...
        if ( M_IO == eMap )  // I/O access
        {
            ...
            if ( cCurrentRomType != 'S' && M_BS == eMap ) {
                ...
                if ( bWrite ) // write cycle?
                {
                    ...
                    if ( cCurrentRomType == 'E' || cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'P' ||
                         cCurrentRomType == 'Q' )
                        RomSwitch( Chipset.Bank_FF ); // CdB for HP: add apples
                }
            }
...
VOID ReadIO( BYTE* a, DWORD d, DWORD s, BOOL bUpdate )
{
    ...
    do {
        switch ( d ) {
            case 0x00:
                *a = ( Chipset.IORam[ d ] & DON ) | Chipset.boffset;
                break;
            case 0x01:
                *a = Chipset.contrast & 0xF;
                break;
            case 0x02:
                *a = Chipset.contrast >> 4;
                break;
            case 0x03:
                *a = 0;
            case 0x04:
            ...
            case 0x0F:
                ...
                if ( ( Chipset.IORam[ CARDCTL ] & ECDT ) == 0 ) {
                    *a = 0; // no cards
                    ...
                    _ASSERT( ( cCurrentRomType != 'E' && cCurrentRomType != 'X' && cCurrentRomType != '2' && cCurrentRomType != 'P' &&
                               cCurrentRomType != 'Q' ) ||
                             Chipset.cards_status == 0xF ); // CdB for HP: add apples
            ...
            case 0x1A:                        // IR CONTROL
                ...
                {
                    Chipset.IORam[ d ] = ( nCurrentClass != 40 ) ? ( Chipset.IORam[ d ] & ~IRI ) // HP39G
            ...
            case 0x37:
                *a = ReadT1();
                break;
            case 0x38:
                Nunpack( a, ReadT2Acc(), s );
                return;
            case 0x39:
                Nunpack( a, ReadT2Acc() >> 4, s );
                return;
            case 0x3A:
                Nunpack( a, ReadT2Acc() >> 8, s );
                return;
            case 0x3B:
                Nunpack( a, ReadT2Acc() >> 12, s );
                return;
            case 0x3C:
                Nunpack( a, ReadT2Acc() >> 16, s );
                return;
            case 0x3D:
                Nunpack( a, ReadT2Acc() >> 20, s );
                return;
            case 0x3E:
                Nunpack( a, ReadT2Acc() >> 24, s );
                return;
            case 0x3F:
                Nunpack( a, ReadT2Acc() >> 28, s );
                return;
            default:
                *a = Chipset.IORam[ d ];
        }
    ...
    if ( rbr_acc )
        CommReceive(); // look for new character
    return;
}
...
VOID WriteIO( BYTE* a, DWORD d, DWORD s )
{
    ...
    do {
        ...
        switch ( d ) {
            ...
            case 0x04:
                Chipset.crc = ( Chipset.crc & 0xfff0 ) | ( c * 0x0001 );
                break;
            case 0x05:
                Chipset.crc = ( Chipset.crc & 0xff0f ) | ( c * 0x0010 );
                break;
            case 0x06:
                Chipset.crc = ( Chipset.crc & 0xf0ff ) | ( c * 0x0100 );
                break;
            case 0x07:
                Chipset.crc = ( Chipset.crc & 0x0fff ) | ( c * 0x1000 );
                break;
            ...
            case 0x0E:
                ...
                if ( ( ( c ^ Chipset.IORam[ d ] ) & ECDT ) != 0 && ( c & ECDT ) == 0 && ( Chipset.IORam[ TIMER2_CTRL ] & RUN ) != 0 ) {
                    ...
                    if ( Chipset.cards_status != ( P2W | P1W | P2C | P1C ) || ( Chipset.IORam[ SRQ2 ] & NINT2 ) == 0 ||
