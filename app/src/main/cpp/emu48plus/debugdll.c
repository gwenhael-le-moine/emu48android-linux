...
BOOL CheckBreakpoint( DWORD dwAddr, DWORD dwRange, UINT nType )
{
    ...
    for ( i = 0; i < wBreakpointCount; ++i ) // scan all breakpoints
    {
        ...
        if ( sBreakpoint[ i ].dwAddr >= dwAddr && sBreakpoint[ i ].dwAddr < dwAddr + dwRange && ( sBreakpoint[ i ].nType & nType ) != 0 )
            return TRUE;
    }
    return bBreak;
}
...
//
VOID CreateBackupBreakpointList( VOID ) { return; }

//
// restore the breakpoint list from the copy
//
VOID RestoreBackupBreakpointList( VOID ) { return; }
...
DECLSPEC BOOL CALLBACK EmuInitLastInstr( WORD wNoInstr,    // @parm number of saved instructions,
...
DECLSPEC BOOL CALLBACK EmuGetLastInstr( WORD* pwNoEntries ) // @parm return number of valid entries in array
{
    WORD i;

    if ( pdwInstrArray == NULL )
        return TRUE; // circular buffer not defined
...
DECLSPEC VOID CALLBACK EmuRunPC( DWORD dwAddressPC ) // @parm breakpoint address
{
    if ( nDbgState != DBG_RUN ) // emulation stopped
    {
...
DECLSPEC BOOL CALLBACK EmuStop( VOID )
{
    if ( pEmuDbgNotify && nDbgState != DBG_STEPINTO ) // emulation running
    {
        ...
        while ( !bDbgNotify )
            Sleep( 0 ); // wait until emulation stopped
    }
...
DECLSPEC VOID CALLBACK EmuCallBackStackNotify( BOOL( CALLBACK* EmuStackNotify )( VOID ) ) // @parm CallBack function notify stack changed
{
...
DECLSPEC DWORD CALLBACK EmuGetRegister( UINT uRegister ) // @parm index of register
{
    DWORD dwResult;
...
DECLSPEC VOID CALLBACK EmuSetRegister( UINT uRegister, // @parm index of register
...
DECLSPEC VOID CALLBACK EmuGetMem( DWORD dwMapAddr, // @parm mapped address of Saturn CPU
...
DECLSPEC VOID CALLBACK EmuSetMem( DWORD dwMapAddr, // @parm mapped address of Saturn CPU
...
DECLSPEC LPBYTE CALLBACK EmuGetRom( DWORD* pdwRomSize ) // @parm return size of ROM in nibbles
{
...
DECLSPEC BOOL CALLBACK EmuSetBreakpoint( DWORD dwAddress,       // @parm breakpoint address to set
                                         UINT nBreakpointType ) // @parm breakpoint type to set
{
    INT i;

    _ASSERT( nBreakpointType == BP_EXEC // illegal breakpoint type
             || nBreakpointType == BP_READ || nBreakpointType == BP_WRITE || nBreakpointType == BP_RPL || nBreakpointType == BP_ACCESS ||
             nBreakpointType == BP_ROM );

    for ( i = 0; i < wBreakpointCount; ++i ) // search for breakpoint
    {
        // breakpoint already set
        if ( sBreakpoint[ i ].dwAddr == dwAddress && sBreakpoint[ i ].nType == nBreakpointType )
            return FALSE;
    }
...
DECLSPEC BOOL CALLBACK EmuClearBreakpoint( DWORD dwAddress,       // @parm breakpoint address to clear
...
DECLSPEC VOID CALLBACK EmuEnableNop3Breakpoint( BOOL bEnable ) // @parm stop on NOP3 opcode
{
...
DECLSPEC VOID CALLBACK EmuEnableDoCodeBreakpoint( BOOL bEnable ) // @parm stop on DOCODE entry
{
...
DECLSPEC VOID CALLBACK EmuEnableRplBreakpoint( BOOL bEnable ) // @parm stop on RPL exit
{
...
DECLSPEC VOID CALLBACK EmuEnableSkipInterruptCode( BOOL bEnable ) // @parm TRUE = skip code instructions
