...
static CONST TokenId eIsGlobalBlock[] = { TOK_GLOBAL, TOK_BACKGROUND, TOK_LCD, TOK_ANNUNCIATOR, TOK_BUTTON, TOK_SCANCODE, TOK_LOCALE };

static CONST TokenId eIsBlock[] = { TOK_IFFLAG, TOK_IFPRESSED, TOK_IFMEM, TOK_ONDOWN, TOK_ONUP };
...
{
    return IDOK == DialogBoxParam( hApp, MAKEINTRESOURCE( IDD_KMLLOG ), hWnd, ( DLGPROC )KMLLogProc, bOkEnabled );
}
...
static VOID CreateKmlList( VOID )
{
    ...
    do {
        ...
        if ( bInvalidPlatform || ( szTitle == NULL ) || ( cKmlType && szTitle[ 0 ] != cKmlType ) || !isModelValid( szTitle[ 0 ] ) ) {
            FreeBlocks( pBlock );
            continue;
        }
...
static LPCTSTR GetStringOf( TokenId eId )
{
    ...
    for ( i = 0; pLexToken[ i ].nLen; ++i ) {
        if ( pLexToken[ i ].eId == eId )
            return pLexToken[ i ].szName;
    }
    return _T("<Undefined>");
}
...
static KmlLine* ParseLine( TokenId eCommand )
{
    ...
    for ( i = 0; pLexToken[ i ].nLen; ++i ) {
        if ( pLexToken[ i ].eId == eCommand )
            break;
    }
    if ( pLexToken[ i ].nLen == 0 )
        return NULL;
...
static KmlLine* IncludeLines( BOOL bInclude, LPCTSTR szFilename )
{
    ...
    nLinesIncludeLevel++;
    PrintfToLog( _T("l%i:%s %s"), nLinesIncludeLevel, ( bInclude ) ? _T("Including") : _T("Parsing"), szFilename );
    InitLex( lpbyBuf );
...
static KmlBlock* IncludeBlocks( BOOL bInclude, LPCTSTR szFilename )
{
    ...
    nBlocksIncludeLevel++;
    PrintfToLog( _T("b%i:%s %s"), nBlocksIncludeLevel, ( bInclude ) ? _T("Including") : _T("Parsing"), szFilename );
    InitLex( lpbyBuf );
    ...
    while ( ( eToken = Lex( LEX_BLOCK ) ) != TOK_NONE ) {
        ...
        if ( eToken == TOK_LOCALE ) {
            ...
            if ( SUBLANGID( wLocId ) == SUBLANG_NEUTRAL ) {
                wKeybId = ( PRIMARYLANGID( wLocId ) != LANG_NEUTRAL ) ? PRIMARYLANGID( wKeybId ) : LANG_NEUTRAL;
            }

            // check if block should be included or skipped
            bIncludeId = bInclude && !bLocaleInc && ( wKeybId == wLocId );

            PrintfToLog( _T("b%i:%s \"Locale %i\""), nBlocksIncludeLevel, ( bIncludeId ) ? _T("Including") : _T("Skipping"), wLocId );
    ...
    while ( pLine ) {
        switch ( pLine->eCommand ) {
            ...
            case TOK_ZOOMXY:
                ...
                break;
            case TOK_COLOR:
                SetLcdColor( ( UINT )pLine->nParam[ 0 ], ( UINT )pLine->nParam[ 1 ], ( UINT )pLine->nParam[ 2 ],
                             ( UINT )pLine->nParam[ 3 ] );
                break;
            case TOK_BITMAP:
                if ( hAnnunDC != NULL ) {
    ...
    while ( pLine ) {
        ...
        switch ( pLine->eCommand ) {
            ...
            case TOK_VIRTUAL:
                ...
                break;
            default:
                PrintfToLog( _T("Command %s Ignored in Block %s %i"), GetStringOf( pLine->eCommand ), GetStringOf( pBlock->eType ),
                             pBlock->nId );
        }
        pLine = pLine->pNext;
    }
...
static VOID FreeLines( KmlLine* pLine )
{
    while ( pLine ) {
        ...
        while ( pLexToken[ i ].nLen ) // search in all token definitions
        {
            ...
            if ( pLexToken[ i ].eId == pLine->eCommand )
                break;
...
VOID KillKML( VOID )
{
    ...
    if ( hPalette ) {
        if ( hWindowDC )
            SelectPalette( hWindowDC, hOldPalette, FALSE );
...
#define LOWADJ 0x10  // color incr. at border
    ...
    VERIFY( hMemBitMap = CreateDIBSection( hWindowDC, &bmi, DIB_RGB_COLORS, ( VOID** )&pbyPixels, NULL, 0 ) );
    if ( hMemBitMap == NULL )
        return;
        ...
        switch ( pButton[ nId ].nType ) {
            case 0: // bitmap key
                if ( pButton[ nId ].bDown ) {
                    BitBlt( hWindowDC, x0, y0, pButton[ nId ].nCx, pButton[ nId ].nCy, hMainDC, pButton[ nId ].nDx, pButton[ nId ].nDy,
                            SRCCOPY );
                } else {
            ...
            case 1: // shift key to right down
                if ( pButton[ nId ].bDown ) {
                    ...
                    MoveToEx( hWindowDC, x0, y0, NULL );
                    LineTo( hWindowDC, x1, y0 );
                    MoveToEx( hWindowDC, x0, y0, NULL );
                    LineTo( hWindowDC, x0, y1 );
                    SelectObject( hWindowDC, GetStockObject( WHITE_PEN ) );
                    MoveToEx( hWindowDC, x1, y0, NULL );
                    LineTo( hWindowDC, x1, y1 );
                    MoveToEx( hWindowDC, x0, y1, NULL );
                    LineTo( hWindowDC, x1 + 1, y1 );
                } else {
                    BitBlt( hWindowDC, x0, y0, pButton[ nId ].nCx, pButton[ nId ].nCy, hMainDC, x0, y0, SRCCOPY );
                }
                break;
...
VOID RefreshButtons( RECT* rc )
{
    UINT i;
    for ( i = 0; i < nButtons; i++ ) {
        if ( pButton[ i ].bDown && rc->right > ( LONG )( pButton[ i ].nOx ) && rc->bottom > ( LONG )( pButton[ i ].nOy ) &&
             rc->left <= ( LONG )( pButton[ i ].nOx + pButton[ i ].nCx ) && rc->top <= ( LONG )( pButton[ i ].nOy + pButton[ i ].nCy ) ) {
...
VOID DrawAnnunciator( UINT nId, BOOL bOn )
{
    ...
    EnterCriticalSection( &csGDILock ); // solving NT GDI problems
    {
        BitBlt( hWindowDC, pAnnunciator[ nId ].nOx, pAnnunciator[ nId ].nOy, pAnnunciator[ nId ].nCx, pAnnunciator[ nId ].nCy, hDC, nSx,
                nSy, SRCCOPY );
        GdiFlush();
    }
    LeaveCriticalSection( &csGDILock );
...
static BOOL ClipButton( UINT x, UINT y, UINT nId )
{
    ...
    return ( pButton[ nId ].nOx <= x ) && ( pButton[ nId ].nOy <= y ) && ( x < ( pButton[ nId ].nOx + pButton[ nId ].nCx ) ) &&
...
VOID MouseMovesTo( UINT nFlags, DWORD x, DWORD y )
{
    ...
    if ( !( nFlags & MK_LBUTTON ) )
        return;                                                // left mouse key not pressed -> quit
    if ( bKeyPressed && !ClipButton( x, y, uLastKeyPressed ) ) // not on last pressed key
        ReleaseAllButtons();                                   // release all buttons
    if ( !bClicking )
        return; // normal emulation key -> quit
...
static VOID ResizeMainBitmap( INT nMul, INT nDiv )
{
    if ( nMul * nDiv > 0 ) // resize main picture
    {
        ...
        EnterCriticalSection( &csGDILock ); // solving NT GDI problems
        {
            ...
            {
                VERIFY( StretchBlt( hMainDC, 0, 0, nWidth, nHeight, hMainDC, 0, 0, Bitmap.bmWidth, Bitmap.bmHeight, SRCCOPY ) );
            } else // expanding bitmap
            {
                ...
                VERIFY( StretchBlt( hMainDC, 0, 0, nWidth, nHeight, hMemDC, 0, 0, Bitmap.bmWidth, Bitmap.bmHeight, SRCCOPY ) );
