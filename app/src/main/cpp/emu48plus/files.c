...
static CONST LPBYTE bySignature[] = { ( CONST LPBYTE ) "Emu48 Document\xFE", ( CONST LPBYTE ) "Emu38 Document\xFE",
                                      ( CONST LPBYTE ) "Emu39 Document\xFE", ( CONST LPBYTE ) "Emu49 Document\xFE",
                                      ( CONST LPBYTE ) "Win48 Document\xFE", ( CONST LPBYTE ) "Win48 Document\xFF" };
...
BOOL CheckForBeepPatch( VOID )
{
    ...
    // known beep patches
    const BEEPPATCH s38[] = {
        { 0x017D0, { 0x8, 0x1, 0xB, 0x1 } }
    };
    const BEEPPATCH s39[] = {
        { 0x017BC, { 0x8, 0x1, 0xB, 0x1 } }
    };
    const BEEPPATCH s48[] = {
        { 0x017A6, { 0x8, 0x1, 0xB, 0x1 } }
    };
    const BEEPPATCH s49[] = {
        {0x4157A, { 0x8, 0x1, 0xB, 0x1 }}, // 1.18/1.19-5/1.19-6
        {0x41609, { 0x8, 0x1, 0xB, 0x1 }}
    }; // 1.24/2.01/2.09
...
BOOL PatchNibble( DWORD dwAddress, BYTE byPatch )
{
    ...
    if ( nodePatch )
        nodePatch->prev = p; // add as previous element
    nodePatch = p;
...
BOOL PatchRom( LPCTSTR szFilename )
{
    ...
    if ( dwFileSizeHigh != 0 || dwFileSizeLow == 0 ) { // file is too large or empty
        CloseHandle( hFile );
        return FALSE;
    }
...
BOOL CrcRom( WORD* pwChk ) // calculate fingerprint of ROM
{
    ...
    if ( pbyRom == NULL )
        return TRUE; // ROM CRC isn't available
...
BOOL MapRom( LPCTSTR szFilename )
{
    ...
    {
        hRomFile =
            CreateFile( szFilename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
        if ( hRomFile == INVALID_HANDLE_VALUE ) {
            bRomRW = FALSE; // ROM not writeable
            hRomFile = CreateFile( szFilename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
                                   FILE_FLAG_SEQUENTIAL_SCAN, NULL );
        }
    } else // writing ROM disabled
    {
        hRomFile = CreateFile( szFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    }
    SetCurrentDirectory( szCurrentDirectory );
    if ( hRomFile == INVALID_HANDLE_VALUE ) {
        hRomFile = NULL;
    ...
    if ( dwRead < sizeof( dwSize ) ) { // file is too small.
...
BOOL MapPort2( LPCTSTR szFilename )
{
    ...
    hPort2File = CreateFile( szFilename, GENERIC_READ | GENERIC_WRITE, bPort2IsShared ? FILE_SHARE_READ : 0, NULL, OPEN_EXISTING,
                             FILE_ATTRIBUTE_NORMAL, NULL );
    if ( hPort2File == INVALID_HANDLE_VALUE ) {
        bPort2Writeable = FALSE;
        hPort2File = CreateFile( szFilename, GENERIC_READ, bPort2IsShared ? ( FILE_SHARE_READ | FILE_SHARE_WRITE ) : 0, NULL, OPEN_EXISTING,
                                 FILE_ATTRIBUTE_NORMAL, NULL );
        if ( hPort2File == INVALID_HANDLE_VALUE ) {
            SetCurrentDirectory( szCurrentDirectory );
            hPort2File = NULL;
    ...
    if ( dwFileSizeHi != 0 || dwFileSizeLo == 0 || ( dwFileSizeLo & ( dwFileSizeLo - 1 ) ) != 0 || ( dwFileSizeLo & 0xFF02FFFF ) != 0 ) {
        UnmapPort2();
        return FALSE;
    }

    hPort2Map = CreateFileMapping( hPort2File, NULL, bPort2Writeable ? PAGE_READWRITE : PAGE_READONLY, 0, dwFileSizeLo, NULL );
    if ( hPort2Map == NULL ) {
        UnmapPort2();
        return FALSE;
    }
...
#define CHECKAREA( s, e ) ( offsetof( CHIPSET, e ) - offsetof( CHIPSET, s ) + sizeof( ( ( CHIPSET* )NULL )->e ) )
    ...
    switch ( byFileSignature[ 14 ] ) {
        case 0xFE: // Win48 2.1 / Emu4x 0.99.x format
...
#if defined _UNICODE
            {
                ...
                MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, szTmp, lBytesRead, szCurrentKml, ARRAYSIZEOF( szCurrentKml ) );
                free( szTmp );
            }
#else
            {
    ...
    if ( lBytesRead != sizeof( lSizeofChipset ) )
        goto read_err;
    ...
    if ( Chipset.Port0Size ) {
        ...
        if ( lBytesRead != Chipset.Port0Size * 2048 )
            goto read_err;

        if ( IsDataPacked( Port0, Chipset.Port0Size * 2048 ) )
            goto read_err;
    }
    ...
    if ( Chipset.Port1Size ) {
        ...
        if ( lBytesRead != Chipset.Port1Size * 2048 )
            goto read_err;

        if ( IsDataPacked( Port1, Chipset.Port1Size * 2048 ) )
            goto read_err;
    }
    ...
    if ( cCurrentRomType == 'S' || cCurrentRomType == 'G' ) {
        ...
        if ( Chipset.wPort2Crc != wPort2Crc && ( Chipset.IORam[ CARDCTL ] & ECDT ) != 0 && ( Chipset.IORam[ TIMER2_CTRL ] & RUN ) != 0 ) {
    ...
    } else // HP38G, HP39/40G, HP49G
    {
        if ( Chipset.Port2Size ) {
            ...
            if ( lBytesRead != Chipset.Port2Size * 2048 )
                goto read_err;

            if ( IsDataPacked( Port2, Chipset.Port2Size * 2048 ) )
                goto read_err;
    ...
    if ( IsDataPacked( Chipset.A, CHECKAREA( A, R4 ) ) )
        goto read_err;
    ...
    if ( pEmuDocumentNotify )
        pEmuDocumentNotify( szCurrentFilename );
#endif
...
BOOL SaveDocument( VOID )
{
...
#if defined _UNICODE
    {
        LPSTR szTmp = ( LPSTR )malloc( nLength );
        if ( szTmp != NULL ) {
            WideCharToMultiByte( CP_ACP, WC_COMPOSITECHECK, szCurrentKml, nLength, szTmp, nLength, NULL, NULL );
    ...
    if ( Chipset.Port0Size )
        WriteFile( hCurrentFile, Port0, Chipset.Port0Size * 2048, &lBytesWritten, NULL );
    if ( Chipset.Port1Size )
        WriteFile( hCurrentFile, Port1, Chipset.Port1Size * 2048, &lBytesWritten, NULL );
    if ( Chipset.Port2Size )
        WriteFile( hCurrentFile, Port2, Chipset.Port2Size * 2048, &lBytesWritten, NULL );
...
BOOL RestoreBackup( VOID )
{
    ...
    if ( bDbgOpen )
        OnToolDebug(); // reopen the debugger
...
BOOL GetOpenFilename( VOID )
{
    ...
    InitializeOFN( &ofn );
    ofn.lpstrFilter = _T("Emu48 Files (*.e38;*.e39;*.e48;*.e49)\0")
...
BOOL GetSaveAsFilename( VOID )
{
    ...
    InitializeOFN( &ofn );
    ofn.lpstrFilter = _T("HP-38 Files (*.e38)\0*.e38\0")
                      _T("HP-39 Files (*.e39)\0*.e39\0")
                      _T("HP-48 Files (*.e48)\0*.e48\0")
                      _T("HP-49 Files (*.e49)\0*.e49\0");
    ...
    if ( cCurrentRomType == 'X' || cCurrentRomType == '2' ||
         cCurrentRomType == 'Q' ) // HP49G/HP48gII/HP49g+/HP50g   // CdB for HP: add apples
    {
...
WORD WriteStack( UINT nStkLevel, LPBYTE lpBuf, DWORD dwSize ) // separated from LoadObject()
{
    ...
    bBinary = ( ( lpBuf[ dwSize + 0 ] == 'H' ) && ( lpBuf[ dwSize + 1 ] == 'P' ) && ( lpBuf[ dwSize + 2 ] == 'H' ) &&
                ( lpBuf[ dwSize + 3 ] == 'P' ) && ( lpBuf[ dwSize + 4 ] == '4' ) &&
                ( lpBuf[ dwSize + 5 ] ==
                  ( ( cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'Q' ) ? '9' : '8' ) ) // CdB for HP: add apples
                && ( lpBuf[ dwSize + 6 ] == '-' ) );
    ...
    if ( bBinary == TRUE ) { // load as binary
...
BOOL LoadObject( LPCTSTR szFilename ) // separated stack writing part
{
    ...
    if ( hFile == INVALID_HANDLE_VALUE )
        return FALSE;
    dwFileSizeLow = GetFileSize( hFile, &dwFileSizeHigh );
    if ( dwFileSizeHigh != 0 ) { // file is too large.
        CloseHandle( hFile );
        return FALSE;
    }
...
BOOL SaveObject( LPCTSTR szFilename ) // separated stack reading part
{
    ...
    pbyHeader = ( ( cCurrentRomType == 'X' || cCurrentRomType == '2' || cCurrentRomType == 'Q' ) ) ? ( LPBYTE )BINARYHEADER49
...
static HBITMAP DecodeBmp( LPBMPFILE pBmp, BOOL bPalette )
{
    ...
    if ( pBmi->bmiHeader.biCompression != BI_RGB ) {
        ...
        dwFileSize += WIDTHBYTES( pBmi->bmiHeader.biWidth * pBmi->bmiHeader.biBitCount ) * labs( pBmi->bmiHeader.biHeight );
    }
    if ( pBmp->dwFileSize < dwFileSize )
        return NULL;

    VERIFY( hBitmap = CreateDIBitmap( hWindowDC, &pBmi->bmiHeader, CBM_INIT, pBmp->pbyFile + pBmfh->bfOffBits, pBmi, DIB_RGB_COLORS ) );
    if ( hBitmap == NULL )
        return NULL;
...
static HBITMAP DecodeGif( LPBMPFILE pBmp, DWORD* pdwTransparentColor, BOOL bPalette )
{
    ...
    if ( ReadGifWord( pBmp, &nWidth ) || ReadGifWord( pBmp, &nHeight ) || ReadGifByte( pBmp, &nInfo ) ||
         ReadGifByte( pBmp, &nBackground ) || ReadGifByte( pBmp, &nZero ) || nZero != 0 )
        goto quit;
    ...
    VERIFY( hBitmap = CreateDIBSection( hWindowDC, &bmi, DIB_RGB_COLORS, ( VOID** )&pbyPixels, NULL, 0 ) );
    if ( hBitmap == NULL )
        goto quit;
    ...
    while ( bDecoding ) {
        ...
        if ( ReadGifByte( pBmp, &nBlockType ) )
            goto quit;
            ...
            case ',': // image
                {
                    ...
                    if ( ReadGifWord( pBmp, &nLeft ) || ReadGifWord( pBmp, &nTop ) || ReadGifWord( pBmp, &nWidth ) ||
                         ReadGifWord( pBmp, &nHeight ) || ReadGifByte( pBmp, &nInfo ) )
                        goto quit;

                    if ( nTop + nHeight > labs( bmi.bmiHeader.biHeight ) || nLeft + nWidth > bmi.bmiHeader.biWidth )
                        goto quit;
                    ...
                    h = 0;
                    v = 0;
                    if ( ReadGifByte( pBmp, &nCodesize ) || ReadGifByte( pBmp, &nBytecount ) )
                        goto quit;
                    ...
                    while ( nBytecount != 0 ) {
                        for ( i = 0; i < nBytecount; ++i ) {
                            ...
                            while ( nBitsAvailable != 0 ) {
                                INT nBitsCopied = ( nBitsNeeded < nBitsAvailable ) ? nBitsNeeded : nBitsAvailable;
                                ...
                                if ( nBitsNeeded == 0 ) {
                                    ...
                                    do {
                                        ...
                                        if ( nCurrCode < nMaxcode ) {
                                            ...
                                            if ( nCurrCode > nMaxcode || nOldCode == -1 )
                                                goto quit;
            ...
            case '!': // extension block
                {
                    ...
                    if ( ReadGifByte( pBmp, &nFunctionCode ) )
                        goto quit;
                    if ( ReadGifByte( pBmp, &nByteCount ) )
                        goto quit;
                    ...
                    if ( nFunctionCode == 0xF9 && nByteCount == 0x04 ) {
                        ...
                        // packed fields
                        if ( ReadGifByte( pBmp, &nPackedFields ) )
                            goto quit;

                        // delay time
                        if ( ReadGifWord( pBmp, &nDummy ) )
                            goto quit;

                        // transparent color index
                        if ( ReadGifByte( pBmp, &nColorIndex ) )
                            goto quit;

                        // transparent color flag set
                        if ( ( nPackedFields & 0x1 ) != 0 ) {
                            if ( pdwTransparentColor != NULL ) {
                                *pdwTransparentColor = RGB( sGlb.bmiColors[ nColorIndex ].rgbRed, sGlb.bmiColors[ nColorIndex ].rgbGreen,
                                                            sGlb.bmiColors[ nColorIndex ].rgbBlue );
                            }
                        }

                        // block terminator (0 byte)
                        if ( !( !ReadGifByte( pBmp, &nDummy ) && nDummy == 0 ) )
                            goto quit;
                    } else // other function
                    {
                        while ( nByteCount != 0 ) {
                            for ( i = 0; i < nByteCount; ++i ) {
                                if ( ReadGifByte( pBmp, &nDummy ) )
                                    goto quit;
                            }
...
HBITMAP LoadBitmapFile( LPCTSTR szFilename, BOOL bPalette )
{
    ...
    do {
        ...
        if ( Bmp.dwFileSize >= 6 && ( memcmp( Bmp.pbyFile, "GIF87a", 6 ) == 0 || memcmp( Bmp.pbyFile, "GIF89a", 6 ) == 0 ) ) {
...
#define ADD_RECTS_COUNT 256
    ...
    dwAlignedWidthBytes = WIDTHBYTES( bi->bmiHeader.biWidth * bi->bmiHeader.biPlanes * bi->bmiHeader.biBitCount );
    ...
    color =
        EncodeColorBits( ( color >> 16 ), dwBlue ) | EncodeColorBits( ( color >> 8 ), dwGreen ) | EncodeColorBits( ( color >> 0 ), dwRed );
