...
BOOL CommOpen( LPTSTR strWirePort, LPTSTR strIrPort )
{
    ...
    if ( lstrcmp( strPort, _T( NO_SERIAL ) ) ) // port defined
    {
        ...
        hComm = CreateFile( szDevice, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED,
                            NULL );
        ...
        if ( hComm != INVALID_HANDLE_VALUE ) {
            ...
            while ( !bReading )
                Sleep( 0 ); // wait for SerialThread started
        } else {
            hComm = NULL;
        }
    }
...
VOID CommSetBaud( VOID )
{
    if ( hComm != NULL ) {
        ...
        uBaudIndex = isModelApple( cCurrentRomType ) ? Chipset.IORam[ BAUD ] : Chipset.IORam[ BAUD ] & 0x7;
...
VOID CommTransmit( VOID )
{
    ...
    if ( bTxChar ) // character to transmit
    {
        ...
        {
            if ( dwBytesRead == 0 )
                nRp = 0;                        // no character received, reset read pointer
