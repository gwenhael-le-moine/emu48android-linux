...
#else
        hr = pKsPropertySet->lpVtbl->Get( pKsPropertySet, &DSPROPSETID_DirectSoundDevice, DSPROPERTY_DIRECTSOUNDDEVICE_DESCRIPTION, NULL, 0,
                                          &sDirectSoundDeviceDescription, sizeof( sDirectSoundDeviceDescription ), &ulBytesReturned );
#endif
...
#else
                hr = pKsPropertySet->lpVtbl->Get( pKsPropertySet, &DSPROPSETID_DirectSoundDevice, DSPROPERTY_DIRECTSOUNDDEVICE_DESCRIPTION,
...
static BOOL CALLBACK DSEnumProc( LPGUID lpGUID, LPCTSTR lpszDesc, LPCTSTR lpszDrvName, LPVOID lpContext )
{
    ...
    if ( lpGUID != NULL ) // NULL only for "Primary Sound Driver"
    {
        ...
        if ( GetInfoFromDSoundGUID( lpGUID, &uDevID ) ) {
            ...
            if ( waveOutGetDevCaps( uDevID, &woc, sizeof( woc ) ) == MMSYSERR_NOERROR && ( woc.dwFormats & WAVE_FORMAT_4M08 ) != 0 ) {
