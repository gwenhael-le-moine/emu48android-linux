...
static VOID ReadReg( LPCTSTR lpSubKey, LPCTSTR lpValueName, LPBYTE lpData, DWORD* pdwSize )
{
    ...
    retCode = RegOpenKeyEx( HKEY_CURRENT_USER, lpKey, 0, KEY_QUERY_VALUE, &hKey );
...
static BOOL WriteReg( LPCTSTR lpSubKey, LPCTSTR lpValueName, DWORD dwType, CONST BYTE* lpData, DWORD cbData )
{
    ...
    retCode = RegCreateKeyEx( HKEY_CURRENT_USER, lpKey, 0, _T(""), REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dwDisposition );
...
static BOOL DelReg( LPCTSTR lpSubKey, LPCTSTR lpValueName )
{
    ...
    retCode = RegOpenKeyEx( HKEY_CURRENT_USER, lpKey, 0, KEY_SET_VALUE, &hKey );
    if ( retCode == ERROR_SUCCESS ) {
...
INT ReadSettingsInt( LPCTSTR lpszSection, LPCTSTR lpszEntry, INT nDefault ) { return ReadInt( lpszSection, lpszEntry, nDefault ); }
