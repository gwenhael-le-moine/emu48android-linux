...
BOOL DLLCreateWnd( LPCTSTR lpszFilename, LPCTSTR lpszPort2Name )
{
    ...
    hWnd = CreateWindow( MAKEINTATOM( classAtom ), _T("Emu48"), WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_OVERLAPPED, CW_USEDEFAULT,
                         CW_USEDEFAULT, rectWindow.right - rectWindow.left, rectWindow.bottom - rectWindow.top, NULL, NULL, hApp, NULL );
...
BOOL DLLDestroyWnd( VOID )
{
    ...
    if ( pEmuClose )
        pEmuClose(); // call notify function
    return FALSE;
}
...
DECLSPEC BOOL CALLBACK EmuCreate( LPCTSTR lpszFilename ) // @parm String with RAM filename
{
...
DECLSPEC BOOL CALLBACK EmuCreateEx( LPCTSTR lpszFilename,   // @parm String with RAM filename
...
DECLSPEC HACCEL CALLBACK EmuAcceleratorTable( HWND* phEmuWnd ) // @parm return of emulator window handle
{
    *phEmuWnd = hWnd;
    return hAccel;
}
...
DECLSPEC BOOL CALLBACK EmuLoadRamFile( LPCTSTR lpszFilename ) // @parm String with RAM filename
{
    BOOL bErr;
...
DECLSPEC BOOL CALLBACK EmuLoadObject( LPCTSTR lpszObjectFilename ) // @parm String with object filename
{
    HANDLE hFile;
    DWORD dwFileSizeLow;
    ...
    if ( nState != SM_RUN )
        goto cancel; // emulator must run to load on object
    if ( WaitForSleepState() )
        goto cancel; // wait for cpu SHUTDN then sleep state
...
DECLSPEC BOOL CALLBACK EmuSaveObject( LPCTSTR lpszObjectFilename ) // @parm String with object filename
{
    BOOL bErr;

    if ( nState != SM_RUN )
        return TRUE; // emulator must run to load on object
    if ( WaitForSleepState() )
        return TRUE; // wait for cpu SHUTDN then sleep state
    _ASSERT( nState == SM_SLEEP );
...
DECLSPEC BYTE CALLBACK EmuCalculatorType( VOID ) { return Chipset.type; }
...
DECLSPEC VOID CALLBACK EmuSimulateKey( BOOL bKeyState, // @parm TRUE = pressed, FALSE = released
...
DECLSPEC VOID CALLBACK EmuPressOn( BOOL bKeyState ) // @parm TRUE = pressed, FALSE = released
{
