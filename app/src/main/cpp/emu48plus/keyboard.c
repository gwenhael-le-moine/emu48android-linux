...
static WORD Keyboard_GetIR( VOID )
{
    ...
    if ( Chipset.out == 0 )
        return 0;
    if ( Chipset.out & 0x001 )
        r |= Chipset.Keyboard_Row[ 0 ];
    if ( Chipset.out & 0x002 )
        r |= Chipset.Keyboard_Row[ 1 ];
    if ( Chipset.out & 0x004 )
        r |= Chipset.Keyboard_Row[ 2 ];
    if ( Chipset.out & 0x008 )
        r |= Chipset.Keyboard_Row[ 3 ];
    if ( Chipset.out & 0x010 )
        r |= Chipset.Keyboard_Row[ 4 ];
    if ( Chipset.out & 0x020 )
        r |= Chipset.Keyboard_Row[ 5 ];
    if ( Chipset.out & 0x040 )
        r |= Chipset.Keyboard_Row[ 6 ];
    if ( Chipset.out & 0x080 )
        r |= Chipset.Keyboard_Row[ 7 ];
    if ( Chipset.out & 0x100 )
        r |= Chipset.Keyboard_Row[ 8 ];
    return r;
}
...
VOID ScanKeyboard( BOOL bActive, BOOL bReset )
{
    ...
    if ( !( bActive || Chipset.Shutdn || Chipset.IR15X || ( Chipset.intk && ( Chipset.IORam[ TIMER2_CTRL ] & RUN ) != 0 ) ) ) {
...
VOID KeyboardEvent( BOOL bPress, UINT out, UINT in )
{
    ...
    if ( in == 0x8000 ) // ON key ?
    {
        ...
        if ( out >= ARRAYSIZEOF( Chipset.Keyboard_Row ) )
            return;
