...
static VOID ( *CONST fnWrState[] )( BYTE a, DWORD d ) = { WrStateIdle, WrStateE8N, WrStateE8D, WrStateE8C, WrState40D,
                                                          WrState20C,  WrState30C, WrStateB8D, WrState60D };
...
static BYTE ( *CONST fnRdState[] )( DWORD d ) = { RdStateData, RdStateId, RdStateQuery, RdStateSR, RdStateXSR };
...
static BYTE RdStateId( DWORD d )
{
    ...
    } else                        // block lock table
    {
        ...
        if ( d >= 4 )
            byData |= 0x02; // set bit 1 on wrong ID adress
    }
    return byData;
}
