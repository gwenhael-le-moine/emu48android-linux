...
static CONST int nCol[] = { IDC_DEBUG_MEM_COL0, IDC_DEBUG_MEM_COL1, IDC_DEBUG_MEM_COL2, IDC_DEBUG_MEM_COL3,
                            IDC_DEBUG_MEM_COL4, IDC_DEBUG_MEM_COL5, IDC_DEBUG_MEM_COL6, IDC_DEBUG_MEM_COL7 };

static CONST TCHAR cHex[] = { _T( '0' ), _T( '1' ), _T( '2' ), _T( '3' ), _T( '4' ), _T( '5' ), _T( '6' ), _T( '7' ),
...
static BOOL GetAddr( HWND hDlg, INT nID, DWORD* pdwAddr, DWORD dwMaxAddr, BOOL bSymbEnable )
{
    ...
    if ( *szBuffer != 0 ) {
        ...
        if ( !bSymbEnable || szBuffer[ 0 ] != _T( '=' ) || RplGetAddr( &szBuffer[ 1 ], pdwAddr ) ) {
...
static VOID SetMappingMenu( HWND hDlg, UINT uID )
{
    ...
    if ( uIDMap != uID )
        dwAdrMem = 0; // view from address 0
...
static __inline VOID InitMemMap( HWND hDlg )
{
    ...
    bActive = ( ID_DEBUG_MEM_NCE1 == uIDMap && GetMemAvail( MEM_NCE1 ) ) || ( ID_DEBUG_MEM_NCE2 == uIDMap && GetMemAvail( MEM_NCE2 ) ) ||
              ( ID_DEBUG_MEM_CE1 == uIDMap && GetMemAvail( MEM_CE1 ) ) || ( ID_DEBUG_MEM_CE2 == uIDMap && GetMemAvail( MEM_CE2 ) ) ||
...
static INT ViewCodeWnd( HWND hWnd, DWORD dwAddress )
{
    ...
    for ( i = 0; i < MAXCODELINES; ++i ) {
        ...
        // remember line of PC
        if ( dwAddress == Chipset.pc )
            nLinePC = i;

        dwAdrLine[ i ] = dwAddress;
        j = wsprintf( szAddress, ( dwAddress == Chipset.pc ) ? _T("%05lX-%c ") : _T("%05lX   "), dwAddress,
                      nRplBreak ? _T( 'R' ) : _T( '>' ) );
...
static BOOL OnMemFollow( HWND hDlg, UINT uID )
{
    if ( ID_DEBUG_MEM_FADDR == uID ) // ask for follow address
    {
        ...
        OnEnterAddress( hDlg, &dwAddress );
        if ( dwAddress == -1 )
            return -1; // return at cancel button
...
{
    if ( uID == uIDMap )
        return -1; // same view, call windows default handler
...
static __inline BOOL OnKeyRightLeft( HWND hWnd, WPARAM wParam )
{
    ...
    if ( wX == MEMWNDMAX )
        return -1; // not IDC_DEBUG_MEM window, default handler
...
static __inline BOOL OnKeyCodeWnd( HWND hDlg, WPARAM wParam )
{
    ...
    if ( wKey == _T( 'G' ) )
        return OnCodeGoAdr( GetParent( hWnd ) ); // goto new address
...
static __inline BOOL OnDrawCodeWnd( LPDRAWITEMSTRUCT lpdis )
{
    ...
    ExtTextOut( lpdis->hDC, ( int )( lpdis->rcItem.left ) + 2, ( int )( lpdis->rcItem.top ), ETO_OPAQUE, ( LPRECT )&lpdis->rcItem, szBuf,
                lstrlen( szBuf ), NULL );
...
VOID UpdateDbgCycleCounter( VOID )
{
    ...
    if ( Chipset.cycles < dwDbgRefCycles )
        ++Chipset.cycles_reserved;
...
BOOL CheckBreakpoint( DWORD dwAddr, DWORD dwRange, UINT nType )
{
    ...
    for ( i = 0; i < wBreakpointCount; ++i ) // scan all breakpoints
    {
        ...
        if ( sBreakpoint[ i ].bEnable && sBreakpoint[ i ].dwAddr >= dwAddr && sBreakpoint[ i ].dwAddr < dwAddr + dwRange &&
...
static __inline HWND CreateToolbar( HWND hWnd )
{
    ...
    hWndToolbar = CreateToolbarEx( hWnd, WS_CHILD | WS_VISIBLE | TBSTYLE_TOOLTIPS, IDR_DEBUG_TOOLBAR, j, hApp, IDR_DEBUG_TOOLBAR, ptbb,
                                   pData->wItemCount, pData->wWidth, pData->wHeight, pData->wWidth, pData->wHeight, sizeof( TBBUTTON ) );
...
static INT_PTR CALLBACK Debugger( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch ( message ) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage( hDlg, IDC_STATIC_REGISTERS, WM_SETFONT, ( WPARAM )GetStockObject( DEFAULT_GUI_FONT ),
                                MAKELPARAM( FALSE, 0 ) );
        ...
        case WM_COMMAND:
            ...
            switch ( LOWORD( wParam ) ) {
                ...
                case ID_DEBUG_STEPOUT:
                    return OnKeyF9( hDlg );
                case ID_DEBUG_BREAK:
                    return OnKeyF11( hDlg );
                case ID_DEBUG_CODE_GOADR:
                    return OnCodeGoAdr( hDlg );
                case ID_DEBUG_CODE_GOPC:
                    return OnCodeGoPC( hDlg );
                case ID_DEBUG_CODE_SETPCTOSELECT:
                    return OnCodeSetPcToSelection( hDlg );
                case ID_DEBUG_CODE_PREVPCO:
                    return OnCodeFindPCO( hDlg, dwAdrLine[ 0 ], -1 );
                case ID_DEBUG_CODE_NEXTPCO:
                    return OnCodeFindPCO( hDlg, dwAdrLine[ 0 ], 1 );
                case ID_BREAKPOINTS_CODEEDIT:
                    return OnEditBreakpoint( hDlg );
                case ID_BREAKPOINTS_CLEARALL:
                    return OnClearAll( hDlg );
                case ID_BREAKPOINTS_NOP3:
                    return OnToggleMenuItem( hDlg, LOWORD( wParam ), &bDbgNOP3 );
                case ID_BREAKPOINTS_DOCODE:
                    return OnToggleMenuItem( hDlg, LOWORD( wParam ), &bDbgCode );
                case ID_BREAKPOINTS_RPL:
                    return OnToggleMenuItem( hDlg, LOWORD( wParam ), &bDbgRPL );
                case ID_TRACE_SETTINGS:
                    return OnTraceSettings( hDlg );
                case ID_TRACE_ENABLE:
                    return OnTraceEnable( hDlg );
                case ID_INFO_LASTINSTRUCTIONS:
                    return OnInfoIntr( hDlg );
                case ID_INFO_PROFILE:
                    return OnProfile( hDlg );
                case ID_INFO_WRITEONLYREG:
                    return OnInfoWoRegister( hDlg );
                case ID_INTR_STEPOVERINT:
                    return OnToggleMenuItem( hDlg, LOWORD( wParam ), &bDbgSkipInt );
                case ID_DEBUG_MEM_GOADR:
                    return OnMemGoAdr( hDlg );
                case ID_DEBUG_MEM_GOPC:
                    return OnMemGoDx( hDlg, Chipset.pc );
                case ID_DEBUG_MEM_GOD0:
                    return OnMemGoDx( hDlg, Chipset.d0 );
                case ID_DEBUG_MEM_GOD1:
                    return OnMemGoDx( hDlg, Chipset.d1 );
                case ID_DEBUG_MEM_GOSTACK:
                    return OnMemGoDx( hDlg, Chipset.rstk[ ( Chipset.rstkp - 1 ) & 7 ] );
                case ID_DEBUG_MEM_FNONE:
                case ID_DEBUG_MEM_FADDR:
                case ID_DEBUG_MEM_FPC:
                case ID_DEBUG_MEM_FD0:
                case ID_DEBUG_MEM_FD1:
                    return OnMemFollow( hDlg, LOWORD( wParam ) );
                case ID_DEBUG_MEM_FIND:
                    return OnMemFind( hDlg );
                case ID_DEBUG_MEM_MAP:
                case ID_DEBUG_MEM_NCE1:
                case ID_DEBUG_MEM_NCE2:
                case ID_DEBUG_MEM_CE1:
                case ID_DEBUG_MEM_CE2:
                case ID_DEBUG_MEM_NCE3:
                    return OnMemMapping( hDlg, LOWORD( wParam ) );
                case ID_DEBUG_MEM_LOAD:
                    return OnMemLoadData( hDlg );
                case ID_DEBUG_MEM_SAVE:
                    return OnMemSaveData( hDlg );
                case ID_DEBUG_MEM_RPLVIEW:
                    return OnRplObjView( hDlg );
                case ID_DEBUG_STACK_PUSH:
                    return OnStackPush( hDlg );
                case ID_DEBUG_STACK_POP:
                    return OnStackPop( hDlg );
                case ID_DEBUG_STACK_MODIFY:
                    return OnStackModify( hDlg );
        ...
        case WM_VKEYTOITEM:
            ...
            {
                case VK_F2:
                    return OnKeyF2( hDlg ); // toggle breakpoint
                case VK_F5:
                    return OnKeyF5( hDlg ); // key run
                case VK_F6:
                    return OnKeyF6( hDlg ); // key step cursor
                case VK_F7:
                    return OnKeyF7( hDlg ); // key step into
                case VK_F8:
                    return OnKeyF8( hDlg ); // key step over
                case VK_F9:
                    return OnKeyF9( hDlg ); // key step out
                case VK_F11:
                    return OnKeyF11( hDlg ); // key break
            }
            ...
            switch ( GetDlgCtrlID( ( HWND )lParam ) ) // calling window
            {
                ...
                case IDC_DEBUG_MEM_COL7:
                    switch ( LOWORD( wParam ) ) {
                        case _T( 'G' ):
                            return OnMemGoAdr( GetParent( ( HWND )lParam ) );
                        case _T( 'F' ):
                            return OnMemFind( GetParent( ( HWND )lParam ) );
                        case VK_RIGHT:
                        case VK_LEFT:
                            return OnKeyRightLeft( ( HWND )lParam, wParam );
                        case VK_NEXT:
                        case VK_PRIOR:
                        case VK_DOWN:
                        case VK_UP:
                            return OnKeyUpDown( ( HWND )lParam, wParam );
                        case VK_ADD:
                        case VK_SUBTRACT:
                            return OnKeyPlusMinus( ( HWND )lParam, wParam );
                    }
                    break;
            }
        ...
        case WM_DRAWITEM:
            if ( wParam == IDC_DEBUG_CODE )
                return OnDrawCodeWnd( ( LPDRAWITEMSTRUCT )lParam );
            break;
...
{
    if ( ( hDlgDebug = CreateDialog( hApp, MAKEINTRESOURCE( IDD_DEBUG ), NULL, ( DLGPROC )Debugger ) ) == NULL )
...
static __inline BOOL OnFindOK( HWND hDlg, BOOL bASCII, DWORD* pdwAddrLast, INT nSearchDir )
{
    ...
    if ( ( lpbySearch = ( LPBYTE )malloc( i * j ) ) != NULL ) {
...
#if defined _UNICODE
        {
            ...
            if ( szTmp != NULL ) {
                WideCharToMultiByte( CP_ACP, WC_COMPOSITECHECK, szTmp, -1, ( LPSTR )lpbySearch, i + 1, NULL, NULL );
                free( szTmp );
            }
        }
#endif
        ...
        if ( bMatch ) {
            ...
            *pdwAddrLast = dwAddr; // last matching address
            ...
            MessageBox( hDlg, _T("Search string not found!"), _T("Find"), MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND );
        }
    }
    return TRUE;
}
        ...
        case WM_COMMAND:
            switch ( LOWORD( wParam ) ) {
                case IDC_FIND_ASCII:
                    ...
                    return TRUE;
                case IDC_FIND_PREV:
                    return OnFindOK( hDlg, bASCII, &dwAddrEntry, -1 );
                case IDC_FIND_NEXT:
                    return OnFindOK( hDlg, bASCII, &dwAddrEntry, 1 );
                case IDCANCEL:
                    DestroyWindow( hDlg );
...
static BOOL OnMemFind( HWND hDlg )
{
    ...
    {
        if ( ( hDlgFind = CreateDialog( hApp, MAKEINTRESOURCE( IDD_FIND ), hDlg, ( DLGPROC )Find ) ) == NULL )
...
#define G2_RATE 0x1B * 2 // CdB for HP: add low speed apples
    ...
    {
        case 'S':
            dwFreq = ( ( SX_RATE + 1 ) * CPU_FREQ / 4 );
            break;
        case 'X':
        case 'G':
        case 'E':
        case 'A':
            dwFreq = ( ( GX_RATE + 1 ) * CPU_FREQ / 4 );
            break;
        case 'P':
        case 'Q':
            dwFreq = ( ( GP_RATE + 1 ) * CPU_FREQ / 4 );
            break;
        case '2':
            dwFreq = ( ( G2_RATE + 1 ) * CPU_FREQ / 4 );
            break;
    }
...
static BOOL OnProfile( HWND hDlg )
{
    ...
    {
        if ( ( hDlgProfile = CreateDialog( hApp, MAKEINTRESOURCE( IDD_PROFILE ), hDlg, ( DLGPROC )Profile ) ) == NULL )
...
static BOOL OnRplObjView( HWND hDlg )
{
    ...
    {
        if ( ( hDlgRplObjView = CreateDialog( hApp, MAKEINTRESOURCE( IDD_RPLVIEW ), hDlg, ( DLGPROC )RplObjView ) ) == NULL )
...
static BOOL OnBrowseSettings( HWND hDlg, HWND hWndFilename )
{
    ...
    ofn.hwndOwner = hDlg;
    ofn.lpstrFilter = _T("HP-Tools Linker File (*.O)\0*.O\0")
                      _T("All Files (*.*)\0*.*\0");
...
static INT_PTR OnNewValue( LPTSTR lpszValue )
{
    ...
    if ( ( nResult = DialogBoxParam( hApp, MAKEINTRESOURCE( IDD_NEWVALUE ), hDlgDebug, ( DLGPROC )NewValue, ( LPARAM )lpszValue ) ) == -1 )
...
static INT_PTR CALLBACK EnterAddr( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    switch ( message ) {
        ...
        case WM_COMMAND:
            ...
            switch ( wParam ) {
                case IDOK:
                    if ( !GetAddr( hDlg, IDC_ENTERADR, ( DWORD* )GetWindowLongPtr( hDlg, GWLP_USERDATA ), 0xFFFFFFFF, disassembler_symb ) )
                        return FALSE;
                    // no break
                case IDCANCEL:
                    EndDialog( hDlg, wParam );
...
static __inline BOOL OnDrawBreakWnd( LPDRAWITEMSTRUCT lpdis )
{
    ...
    // write Text
    SendMessage( lpdis->hwndItem, LB_GETTEXT, lpdis->itemID, ( LPARAM )szBuf );
    ExtTextOut( lpdis->hDC, ( int )( lpdis->rcItem.left ) + 17, ( int )( lpdis->rcItem.top ), ETO_OPAQUE, ( LPRECT )&lpdis->rcItem, szBuf,
                lstrlen( szBuf ), NULL );
    ...
    BitBlt( lpdis->hDC, lpdis->rcItem.left + 2, lpdis->rcItem.top + 2, 11, lpdis->rcItem.bottom - lpdis->rcItem.top, hdcMem,
            sBreakpoint[ lpdis->itemData ].bEnable ? 0 : 10, 0, SRCCOPY );
...
static INT_PTR CALLBACK EditBreakpoint( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch ( message ) {
        case WM_INITDIALOG:
            // font settings
            SendDlgItemMessage( hDlg, IDC_STATIC_BREAKPOINT, WM_SETFONT, ( WPARAM )GetStockObject( DEFAULT_GUI_FONT ),
                                MAKELPARAM( FALSE, 0 ) );
            SendDlgItemMessage( hDlg, IDC_BREAKEDIT_ADD, WM_SETFONT, ( WPARAM )GetStockObject( DEFAULT_GUI_FONT ), MAKELPARAM( FALSE, 0 ) );
            SendDlgItemMessage( hDlg, IDC_BREAKEDIT_DELETE, WM_SETFONT, ( WPARAM )GetStockObject( DEFAULT_GUI_FONT ),
                                MAKELPARAM( FALSE, 0 ) );
                    ...
                    for ( nItem = wBreakpointCount - 1; nItem >= 0; --nItem ) {
                        ...
                        if ( SendMessage( hWnd, LB_GETSEL, nItem, 0 ) > 0 ) {
                            ...
                            for ( j = 0; j < wBreakpointCount; ++j ) {
                                ...
                                if ( k > i )
                                    SendMessage( hWnd, LB_SETITEMDATA, j, k - 1 );
                            }
        ...
        case WM_DRAWITEM:
            if ( wParam == IDC_BREAKEDIT_WND )
                return OnDrawBreakWnd( ( LPDRAWITEMSTRUCT )lParam );
            break;
...
VOID LoadBreakpointList( HANDLE hFile ) // NULL = clear breakpoint list
{
    ...
    if ( hFile )
        ReadFile( hFile, &wBreakpointCount, sizeof( wBreakpointCount ), &lBytesRead, NULL );
...
static BOOL OnBrowseLoadMem( HWND hDlg )
{
    ...
    ofn.lStructSize = sizeof( OPENFILENAME );
    ofn.hwndOwner = hDlg;
    ofn.lpstrFilter = _T("Memory Dump Files (*.MEM)\0*.MEM\0")
                      _T("All Files (*.*)\0*.*\0");
        ...
        case WM_COMMAND:
            switch ( LOWORD( wParam ) ) {
                ...
                case IDOK:
                    ...
                    if ( !GetAddr( hDlg, IDC_DEBUG_DATA_STARTADDR, &dwStartAddr, 0xFFFFF, FALSE ) || dwStartAddr == -1 )
                        return FALSE;
        ...
        case WM_COMMAND:
            switch ( LOWORD( wParam ) ) {
                ...
                case IDOK:
                    ...
                    if ( !GetAddr( hDlg, IDC_DEBUG_DATA_STARTADDR, &dwStartAddr, 0xFFFFF, FALSE ) || dwStartAddr == -1 )
                        return FALSE;
                    if ( !GetAddr( hDlg, IDC_DEBUG_DATA_ENDADDR, &dwEndAddr, 0xFFFFF, FALSE ) || dwEndAddr == -1 )
                        return FALSE;
...
static VOID StartTrace( VOID )
{
    if ( hLogFile == NULL ) {
        ...
        hLogFile = CreateFile( szTraceFilename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL,
                               ( uTraceMode == TRACE_FILE_NEW ) ? CREATE_ALWAYS : OPEN_ALWAYS, 0, NULL );
...
static __inline void __cdecl PrintTrace( LPCTSTR lpFormat, ... )
{
...
#if defined _UNICODE
    {
        ...
        if ( szTmp != NULL ) {
            WideCharToMultiByte( CP_ACP, WC_COMPOSITECHECK, szTmp, -1, ( LPSTR )cOutput, sizeof( cOutput ), NULL, NULL );
            free( szTmp );
        }
    }
#endif
...
static VOID OutTrace( VOID )
{
    ...
    if ( hLogFile != NULL ) // log file opened
    {
        if ( bTraceReg ) // show regs
        {
            ...
            PrintTrace( _T("  R4=%s D0=%05X  D1=%05X  P=%X  CY=%d  Mode=%c  OUT=%03X  IN=%04X\r\n"), RegToStr( Chipset.R4, 16 ), Chipset.d0,
                        Chipset.d1, Chipset.P, Chipset.carry, Chipset.mode_dec ? _T( 'D' ) : _T( 'H' ), Chipset.out, Chipset.in );
...
static BOOL OnBrowseTraceSettings( HWND hDlg )
{
    ...
    ofn.lStructSize = sizeof( OPENFILENAME );
    ofn.hwndOwner = hDlg;
    ofn.lpstrFilter = _T("Trace Log Files (*.log)\0*.log\0")
                      _T("All Files (*.*)\0*.*\0");
