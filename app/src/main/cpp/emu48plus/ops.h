...
static __inline LPBYTE FASTPTR( DWORD d )
{
    ...
    if ( !( Chipset.IOCfig && ( ( d & 0xFFFC0 ) == Chipset.IOBase ) ) && RMap[ u ] != NULL // page valid
...
static __inline void Nbit0( BYTE* a, UINT b ) { a[ b >> 2 ] &= ~( 1 << ( b & 3 ) ); }

static __inline void Nbit1( BYTE* a, UINT b ) { a[ b >> 2 ] |= 1 << ( b & 3 ); }

static __inline void Tbit0( BYTE* a, UINT b ) { Chipset.carry = ( ( a[ b >> 2 ] & ( 1 << ( b & 3 ) ) ) == 0 ); }

static __inline void Tbit1( BYTE* a, UINT b ) { Chipset.carry = ( ( a[ b >> 2 ] & ( 1 << ( b & 3 ) ) ) != 0 ); }
