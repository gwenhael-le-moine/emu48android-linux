...
VOID o807( LPBYTE I ) // SHUTDN
{
    ...
    if ( ( w.in & 0x7FFF ) == 0 && bShutdn ) // shut down only when enabled
    {
        ...
        if ( ( cCurrentRomType == 'E' || cCurrentRomType == 'G' || cCurrentRomType == 'X' || cCurrentRomType == '2' ||
               cCurrentRomType == 'P' || cCurrentRomType == 'Q' ) // CdB for HP: add apples
             && ( w.IORam[ BITOFFSET ] & DON ) == 0 && ( ( w.IORam[ CARDCTL ] & ( ECDT | RCDT ) ) != ( ECDT | RCDT ) ) ) {
            ...
            if ( cCurrentRomType != 'G' || ( ( w.IORam[ LINECOUNT + 1 ] & DA19 ) && ( w.pc & 0x80000 ) ) ) {
