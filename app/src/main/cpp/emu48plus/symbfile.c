 ...
 *
 *   Copyright (C) 2008 Christoph Giesselink
 *
 */
#include "pch.h"
#include "emu48.h"
    ...
    if ( hFile != INVALID_HANDLE_VALUE ) {
        ...
        for ( dwPageIndex = 256, dwSymb = 0; bSucc && dwSymb < dwNoSymbols; dwPageIndex += 42 ) {
            if ( dwPageIndex >= 256 ) // read complete page
            {
                ...
                if ( dwFilePos > dwFileLength || dwBytesRead != sizeof( byPage ) || memcmp( byPage, SYMB_ID, 4 ) != 0 ) {
                    bSucc = FALSE;
                    break;
                }
            ...
            if ( bSymbol ) // this is the 42 byte symbol set
            {
                ...
                if ( bSymbol )
                    ++dwResolvedSymb; // added resolved symbol
...
#if defined _UNICODE
                    {
                        MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, ( LPCSTR )byPage + dwPageIndex, 36, szSymbolName,
                                             ARRAYSIZEOF( szSymbolName ) );
        ...
        bSucc = bSucc && ( dwFilePos <= dwFileLength ) && ( dwNoSymbols == ( dwResolvedSymb + dwNoReferences ) );
