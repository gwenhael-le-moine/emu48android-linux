 ...
 *
 *   Copyright (C) 2013 Christoph Giesselink
 *
 */
#include "pch.h"
#include "emu48.h"
...
static DWORD GetSoundBufSize( VOID )
{
    ...
    EnterCriticalSection( &csSoundLock );
    {
        ...
        dwNoSamples = ( psTail == NULL ) ? 0 : ( psTail->dwIndex - psHead->dwIndex ) + 1;
    }
    LeaveCriticalSection( &csSoundLock );
    return dwNoSamples;
}
...
static BOOL AddSamples( DWORD dwSamples, BYTE byLevel )
{
    ...
    EnterCriticalSection( &csSoundLock );
    {
        ...
        do {
            ...
            dwBufSamples = ( psData != NULL ) ? ( psData->dwBufferLength - psData->dwPosition ) : 0;
            ...
            if ( dwBufSamples == 0 ) // sample buffer is full
            {
                ...
                _ASSERT( psData != NULL && psData->pbyData != NULL && psData->dwPosition == 0 );
...
{
    WAVEOUTCAPS woc;
    return waveOutGetDevCaps( uDeviceID, &woc, sizeof( woc ) ) == MMSYSERR_NOERROR && ( woc.dwFormats & WAVE_FORMAT_4M08 ) != 0;
}
...
VOID SoundOut( CHIPSET* w, WORD wOut )
{
    ...
    dwDiffCycles = ( cCurrentRomType == 'S' ) ? ( dwDiffSatCycles * 26 ) / 25  // Clarke * 1.04
...
#if defined DEBUG_SOUND
    {
        ...
        wsprintf( buffer, _T("State %u: Time = %I64u us  f = %u Hz, Time = %I64u us  f = %u Hz\n"), wOut >> 11, lDuration,
                  ( DWORD )( 1000000 / 2 / lDuration ), ( QWORD )dwSamples * 1000000 / SAMPLES_PER_SEC, SAMPLES_PER_SEC / 2 / dwSamples );
