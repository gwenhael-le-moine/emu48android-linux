...
static DWORD CalcT2( VOID ) // calculate timer2 value
{
    ...
    if ( bStarted )          // timer2 running
    {
        ...
        dwT2 -= ( DWORD )( ( ( lT2Act.QuadPart - lT2Ref.QuadPart ) * T2_FREQ ) / lFreq.QuadPart );
        ...
        if ( Chipset.inte && ( dwT2 & 0x80000000 ) != 0 && ( !Chipset.Shutdn || ( Chipset.IORam[ TIMER2_CTRL ] & WKE ) ) &&
...
static VOID CheckT1( BYTE nT1 )
{
    ...
    if ( ( !Chipset.Shutdn || ( Chipset.IORam[ TIMER1_CTRL ] & WKE ) ) && ( Chipset.IORam[ TIMER1_CTRL ] & INTR ) ) {
...
static VOID CheckT2( DWORD dwT2 )
{
    ...
    if ( ( !Chipset.Shutdn || ( Chipset.IORam[ TIMER2_CTRL ] & WKE ) ) && ( Chipset.IORam[ TIMER2_CTRL ] & INTR ) ) {
        Chipset.SoftInt = TRUE;
        bInterrupt = TRUE;
    }
...
VOID SetT1( BYTE byValue )
{
    ...
    LeaveCriticalSection( &csT1Lock );
    if ( bEqual )
        return; // same value doesn't restart timer period
