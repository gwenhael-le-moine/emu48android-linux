 ...
 *   Copyright (C) 2002 Christoph Giesselink
 *
 */
#include "pch.h"
#include "resource.h"
...
#define GRAYMASK( c )                                                                                                                      \
    ( ( ( ( ( c ) - 1 ) >> 1 ) << 24 ) | ( ( ( ( c ) - 1 ) >> 1 ) << 16 ) | ( ( ( ( c ) - 1 ) >> 1 ) << 8 ) | ( ( ( ( c ) - 1 ) >> 1 ) ) )

#define DIBPIXEL4( d, p )                                                                                                                  \
    *( ( DWORD* )( d ) ) = ( ( *( ( DWORD* )( d ) ) & dwGrayMask ) << 1 ) | ( p );                                                         \
    *( ( LPBYTE* )&( d ) ) += 4
...
VOID UpdateMainDisplay( VOID )
{
    ...
    EnterCriticalSection( &csGDILock ); // solving NT GDI problems
    {
        ...
        StretchBlt( hWindowDC, nLcdX, nLcdY + Chipset.d0size * nLcdZoom, 131 * nLcdZoom * nGdiXZoom,
                    MAINSCREENHEIGHT * nLcdZoom * nGdiYZoom, hLcdDC, Chipset.boffset, Chipset.d0size, 131, MAINSCREENHEIGHT, SRCCOPY );
        GdiFlush();
    }
...
VOID UpdateMenuDisplay( VOID )
{
    ...
    {
        // CdB for HP: add 64/80 line display for apples
        StretchBlt( hWindowDC, nLcdX, nLcdY + ( MAINSCREENHEIGHT + Chipset.d0size ) * nLcdZoom * nGdiYZoom, 131 * nLcdZoom * nGdiXZoom,
                    MENUHEIGHT * nLcdZoom * nGdiYZoom, hLcdDC, 0, ( MAINSCREENHEIGHT + Chipset.d0size ), 131, MENUHEIGHT, SRCCOPY );
        GdiFlush();
    }
    LeaveCriticalSection( &csGDILock );
    return;
...
VOID RefreshDisp0()
{
    ...
    EnterCriticalSection( &csGDILock ); // solving NT GDI problems
    {
        StretchBlt( hWindowDC, nLcdX, nLcdY, 131 * nLcdZoom * nGdiXZoom, Chipset.d0size * nLcdZoom * nGdiYZoom, hLcdDC, Chipset.d0offset, 0,
...
VOID WriteToMainDisplay( LPBYTE a, DWORD d, UINT s )
{
    ...
    if ( bGrayscale )
        return; // no direct writing in grayscale mode
    ...
    if ( !( y0 >= ( INT )Chipset.d0size && y0 < ( INT )( MAINSCREENHEIGHT + Chipset.d0size ) ) )
        return;
    ...
    while ( s-- ) // loop for nibbles to write
    {
        ...
        if ( ( ( INT )x == lWidth ) && s ) // end of display line
        {
            ...
            if ( y == ( INT )MAINSCREENHEIGHT + Chipset.d0size - 1 )
                break;
            x = 0; // first coloumn
            ++y;   // next row
    ...
    EnterCriticalSection( &csGDILock ); // solving NT GDI problems
    {
        StretchBlt( hWindowDC, nLcdX + x0 * nLcdZoom * nGdiXZoom, nLcdY + y0 * nLcdZoom * nGdiYZoom, x * nLcdZoom * nGdiXZoom,
                    ( y - y0 ) * nLcdZoom * nGdiYZoom, hLcdDC, x0 + Chipset.boffset, y0, x, y - y0,
                    SRCCOPY ); // CdB for HP: add 64/80 line display for apples
        GdiFlush();
    }
    LeaveCriticalSection( &csGDILock );
    return;
...
VOID WriteToMenuDisplay( LPBYTE a, DWORD d, UINT s )
{
    ...
    if ( !( y0 >= ( UINT )( Chipset.d0size + MAINSCREENHEIGHT ) && y0 < ( UINT )( SCREENHEIGHT ) ) )
        return;
    ...
    EnterCriticalSection( &csGDILock ); // solving NT GDI problems
    {
        StretchBlt( hWindowDC, nLcdX, nLcdY + y0 * nLcdZoom * nGdiYZoom, ( 131 * nLcdZoom ) * nGdiXZoom, ( y - y0 ) * nLcdZoom * nGdiYZoom,
...
VOID ResizeWindow( VOID )
{
    if ( hWnd != NULL ) // if window created
    {
        ...
        AdjustWindowRect( &rectWindow, ( DWORD )GetWindowLongPtr( hWnd, GWL_STYLE ),
                          GetMenu( hWnd ) != NULL || IsRectEmpty( &rectWindow ) );
        SetWindowPos( hWnd, bAlwaysOnTop ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, rectWindow.right - rectWindow.left,
                      rectWindow.bottom - rectWindow.top, SWP_NOMOVE );
        ...
        if ( rectClient.bottom < ( LONG )nBackgroundH ) {
            ...
            SetWindowPos( hWnd, NULL, 0, 0, rectWindow.right - rectWindow.left, rectWindow.bottom - rectWindow.top,
