...
static const LPCTSTR szLicence = _T("This program is free software; you can redistribute it and/or modify\r\n")
        ...
        if ( !bTitleBar && GetMenu( hWnd ) == NULL ) {
            if ( hRgn == NULL ) {
                EnterCriticalSection( &csGDILock ); // solving NT GDI problems
                {
                    ...
                    hRgn = CreateRgnFromBitmap( ( HBITMAP )GetCurrentObject( hMainDC, OBJ_BITMAP ), dwTColor, dwTColorTol );
...
static VOID SetCommList( HWND hDlg, LPCTSTR szWireSetting, LPCTSTR szIrSetting )
{
    ...
    if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, _T("Hardware\\DeviceMap\\SerialComm"), 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hKey ) ==
         ERROR_SUCCESS ) {
        HANDLE hComm;
        ...
        do {
            ...
            lRet = RegEnumValue( hKey, dwEnumVal++, cKey, &dwKeySize, NULL, &dwType, ( LPBYTE )cData, &dwDataSize );
            ...
            switch ( ( ( LPNMHDR )lParam )->code ) {
                case PSN_KILLACTIVE:
                    if ( Chipset.Port1Size &&
                         ( cCurrentRomType != 'X' || cCurrentRomType != '2' || cCurrentRomType != 'Q' ) ) // CdB for HP: add apples
                    {
                    ...
                    if ( bPort2CfgChange ) // slot2 configuration changed
                    {
                        ...
                        if ( cCurrentRomType ) // ROM defined
                        {
                            ...
                            if ( ( bPort2AttChange || Chipset.wPort2Crc != wPort2Crc ) && ( Chipset.IORam[ CARDCTL ] & ECDT ) != 0 &&
...
static LRESULT OnPaint( HWND hWindow )
{
    ...
    if ( hMainDC != NULL ) {
        ...
        EnterCriticalSection( &csGDILock ); // solving NT GDI problems
        {
            ...
            BitBlt( hPaintDC, Paint.rcPaint.left, Paint.rcPaint.top, Paint.rcPaint.right - Paint.rcPaint.left,
                    Paint.rcPaint.bottom - Paint.rcPaint.top, hMainDC, rcMainPaint.left, rcMainPaint.top, SRCCOPY );

            // CdB for HP: add apples display stuff
            SetWindowOrgEx( hPaintDC, nBackgroundX, nBackgroundY, NULL );

            // redraw header display area
            StretchBlt( hPaintDC, nLcdX, nLcdY, 131 * nLcdZoom * nGdiXZoom, Chipset.d0size * nLcdZoom * nGdiYZoom, hLcdDC, Chipset.d0offset,
                        0, 131, Chipset.d0size, SRCCOPY );
            // redraw main display area
            StretchBlt( hPaintDC, nLcdX, nLcdY + Chipset.d0size * nLcdZoom * nGdiYZoom, 131 * nLcdZoom * nGdiXZoom,
                        nLines * nLcdZoom * nGdiYZoom, hLcdDC, Chipset.boffset, Chipset.d0size, 131, nLines, SRCCOPY );
            // redraw menu display area
            StretchBlt( hPaintDC, nLcdX, nLcdY + ( nLines + Chipset.d0size ) * nLcdZoom * nGdiYZoom, 131 * nLcdZoom * nGdiXZoom,
                        MENUHEIGHT * nLcdZoom * nGdiYZoom, hLcdDC, 0, ( nLines + Chipset.d0size ), 131, MENUHEIGHT, SRCCOPY );
            GdiFlush();
        }
        LeaveCriticalSection( &csGDILock );
...
static LRESULT OnInitMenu( HMENU hMenu )
{
    ...
    BOOL bStackEnable =
        cCurrentRomType != '6' && cCurrentRomType != 'A' && cCurrentRomType != 'E' && cCurrentRomType != 'P'; // CdB for HP: add apples
...
#define PALVERSION 0x300
            ...
            hBmp = CreateCompatibleBitmap( hLcdDC, 131 * nLcdZoom * nGdiXZoom,
                                           SCREENHEIGHT * nLcdZoom * nGdiYZoom ); // CdB for HP: add apples display stuff
            ...
            EnterCriticalSection( &csGDILock ); // solving NT GDI problems
            {
                ...
                StretchBlt( hBmpDC, 0, 0, 131 * nLcdZoom * nGdiXZoom, Chipset.d0size * nLcdZoom * nGdiYZoom, hLcdDC, Chipset.d0offset, 0,
                            131, Chipset.d0size, SRCCOPY );
                // copy main display area
                StretchBlt( hBmpDC, 0, Chipset.d0size * nLcdZoom * nGdiYZoom, 131 * nLcdZoom * nGdiXZoom, nLines * nLcdZoom * nGdiYZoom,
                            hLcdDC, Chipset.boffset, Chipset.d0size, 131, nLines, SRCCOPY );
                // copy menu display area
                StretchBlt( hBmpDC, 0, ( nLines + Chipset.d0size ) * nLcdZoom * nGdiYZoom, 131 * nLcdZoom * nGdiXZoom,
                            MENUHEIGHT * nLcdZoom * nGdiYZoom, hLcdDC, 0, ( nLines + Chipset.d0size ), 131, MENUHEIGHT, SRCCOPY );
                GdiFlush();
            }
            LeaveCriticalSection( &csGDILock );
            ...
            if ( ( hClipObj = GlobalAlloc( GMEM_MOVEABLE, dwLen ) ) != NULL ) {
                ...
                GetDIBits( hBmpDC, hBmp, 0, lpbi->biHeight, ( LPBYTE )lpbi + dwLen - dwSizeImage, ( LPBITMAPINFO )lpbi, DIB_RGB_COLORS );
...
static LRESULT OnObjectLoad( VOID )
{
    ...
    if ( bLoadObjectWarning ) {
        UINT uReply = YesNoCancelMessage( _T("Warning: Trying to load an object while the emulator is busy\n")
...
static INT_PTR CALLBACK Disasm( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch ( message ) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage( hDlg, IDC_DISASM_MODE_TEXT, WM_SETFONT, ( WPARAM )GetStockObject( DEFAULT_GUI_FONT ),
                                MAKELPARAM( FALSE, 0 ) );
    ...
    if ( GetMenu( hWnd ) == NULL ) // no main window menu
    {
        ...
        if ( bContextMenu ) // call the context menu
        {
            ...
            if ( ( hMenu = LoadMenu( hApp, MAKEINTRESOURCE( IDM_MENU ) ) ) != NULL ) {
                ...
                TrackPopupMenu( GetSubMenu( hMenu, 0 ), TPM_LEFTALIGN | TPM_LEFTBUTTON, pt.x, pt.y, 0, hWnd, NULL );
...
{
    if ( nMacroState == MACRO_PLAY )
        return 0; // playing macro
    if ( nState == SM_RUN )
        ...
        case CDID_FILENAME:
            ...
            if ( bDocumentAvail && psCDS->cbData > 0 && psCDS->lpData != NULL ) {
...
#else
                {
                    ...
                    WideCharToMultiByte( CP_ACP, WC_COMPOSITECHECK, ( LPCWSTR )psCDS->lpData, -1, szAscFilename,
                                         ARRAYSIZEOF( szAscFilename ), NULL, NULL );
...
LRESULT CALLBACK MainWndProc( HWND hWindow, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    switch ( uMsg ) {
        ...
        case WM_INITMENU:
            return OnInitMenu( ( HMENU )wParam );
        case WM_DROPFILES:
        ...
        case WM_COMMAND:
            switch ( LOWORD( wParam ) ) {
                ...
                case ID_BACKUP_SAVE:
                    return OnBackupSave();
                case ID_BACKUP_RESTORE:
                    return OnBackupRestore();
                case ID_BACKUP_DELETE:
                    return OnBackupDelete();
                case ID_OBJECT_LOAD:
                    return OnObjectLoad();
                case ID_OBJECT_SAVE:
                    return OnObjectSave();
                ...
                case ID_TOOL_DEBUG:
                    return OnToolDebug();
                case ID_TOOL_MACRO_RECORD:
                    return OnToolMacroNew();
                case ID_TOOL_MACRO_PLAY:
                    return OnToolMacroPlay();
                case ID_TOOL_MACRO_STOP:
                    return OnToolMacroStop();
                case ID_TOOL_MACRO_SETTINGS:
                    return OnToolMacroSettings();
                case ID_HELP_TOPICS:
                    return OnTopics();
                case ID_ABOUT:
                    return OnAbout();
        ...
        case WM_NCMOUSEMOVE:
            return OnNcMouseMove( ( UINT )wParam, LOWORD( lParam ), HIWORD( lParam ) );
        case WM_KEYUP:
            return OnKeyUp( ( int )wParam, lParam );
        case WM_KEYDOWN:
            return OnKeyDown( ( int )wParam, lParam );
#if !defined _USRDLL // not in DLL version
        case WM_COPYDATA:
            return OnCopyData( ( PCOPYDATASTRUCT )lParam );
#endif
    }
...
int WINAPI WinMain( HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow )
{
    ...
    if ( !( classAtom = RegisterClass( &wc ) ) ) {
        AbortMessage( _T("CEmu48 class registration failed.\n")
    ...
    if ( bSingleInstance && ( hWnd = FindWindow( MAKEINTATOM( classAtom ), NULL ) ) != NULL ) {
        ...
        if ( nArgc >= 2 ) // use decoded parameter line
        {
...
#else
            {
                ...
                if ( sCDS.lpData != NULL ) {
                    MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, szBufferFilename, dwLength, ( LPWSTR )sCDS.lpData, sCDS.cbData );
                } else {
    ...
    {
        typedef const BOOL( WINAPI * LPFN_SPI )( HANDLE hProcess, INT ProcessInformationClass, LPVOID ProcessInformation,
    ...
    hWnd = CreateWindow( MAKEINTATOM( classAtom ), _T("Emu48"), STYLE_TITLE, CW_USEDEFAULT, CW_USEDEFAULT,
                         rectWindow.right - rectWindow.left, rectWindow.bottom - rectWindow.top, NULL, NULL, hApp, NULL );
    ...
    while ( nState != nNextState )
        Sleep( 0 ); // wait for thread initialized

    idDdeInst = 0; // initialize DDE server
    if ( DdeInitialize( &idDdeInst, ( PFNCALLBACK )&DdeCallback,
                        APPCLASS_STANDARD | CBF_FAIL_EXECUTES | CBF_FAIL_ADVISES | CBF_SKIP_REGISTRATIONS | CBF_SKIP_UNREGISTRATIONS,
                        0 ) ) {
...
start:
    if ( bStartupBackup )
        SaveBackup(); // make a RAM backup at startup
    if ( pbyRom )
        SwitchToState( SM_RUN );

    while ( GetMessage( &msg, NULL, 0, 0 ) ) {
        if ( !TranslateAccelerator( hWnd, hAccel, &msg ) && ( hDlgDebug == NULL || !IsDialogMessage( hDlgDebug, &msg ) ) &&
